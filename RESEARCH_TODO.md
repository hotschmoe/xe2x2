...
(av), 4-acc no-SLM 120 (aw), B-db+ca.ca 105-107 (ax)
vs W8A8 46. av is the hand floor (~2.1x W8A8).
B-db+ca.ca is a tax vs A-db. GRF256 still zebin 128.
Vectorized RMSNorm-quant fuse (au) is 72 us;
two-launch WG-256+GEMM stays. Next: ngen null-dest
prefetch on sc8db (no extra GRF), or stop M=64
load-path chasing. Loop every 20m.
