...
mkdir -p "$ROOT/results/p2"
{
  ...
  timeout --signal=TERM --kill-after=8 45 docker run --rm --device /dev/dri \
    ...
    "$IMG" \
    --standalone --nproc-per-node=2 /work/parallel/tp2/bench_sr256.py
  rc=$?
  set -e
  echo "TIMEOUT_OR_EXIT rc=$rc"
  if [ "$rc" -eq 124 ] || [ "$rc" -eq 137 ]; then
    echo "RESULT op=sendrecv name=prefill_256h HANG timeout=45s p2p=0"
  fi
} | tee "$OUT"
echo "wrote $OUT"
exit 0
