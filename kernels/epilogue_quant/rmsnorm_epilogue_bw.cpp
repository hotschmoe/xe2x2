// K5 bandwidth RMSNorm-epilogue s8. Backend: sycl+l0. AOT intel_gpu_bmg_g31.
// Standalone icpx -fsycl. One work-group per row, WG=256, group reduce.
// CONFIG prior: naive one-WI fused was 830 us at M=1 K=5120 (K5 2026-09-02s).
// Same contract: RMSNorm gamma=1 eps=1e-6, symmetric s8 qmax=127, per-row absmax.
// Path A: 2 launches (vectorized rmsnorm, then quant).
// Path B: 1 launch (fused). Rank us and launch count.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "rmsnorm_epilogue_bw: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_rmsnorm(sycl::queue &q, const sycl::half *xin,
    ...
    return q.parallel_for<RmsBwName>(
        sycl::nd_range<1>({size_t(m) * size_t(kWg)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) {
            const int row = int(it.get_group(0));
            const int lid = int(it.get_local_id(0));
            const sycl::half *in = xin + row * k;
            float ss = 0.f;
            for (int j = lid; j < k; j += kWg) {
            ...
            sycl::half *out = yout + row * k;
            for (int j = lid; j < k; j += kWg)
                out[j] = sycl::half(float(in[j]) * inv);
        });
}

sycl::event launch_quant(sycl::queue &q, const sycl::half *yin, int8_t *qout,
                         float *sout, int m, int k) {
    return q.parallel_for<QuantBwName>(
        sycl::nd_range<1>({size_t(m) * size_t(kWg)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) {
            const int row = int(it.get_group(0));
            const int lid = int(it.get_local_id(0));
            const sycl::half *in = yin + row * k;
            float amax = 0.f;
            for (int j = lid; j < k; j += kWg) {
                const float y = float(in[j]);
                const float ay = y < 0.f ? -y : y;
                if (ay > amax)
                    amax = ay;
            }
...
sycl::event launch_fused(sycl::queue &q, const sycl::half *xin, int8_t *qout,
                         float *sout, int m, int k) {
    return q.parallel_for<FusedBwName>(
        sycl::nd_range<1>({size_t(m) * size_t(kWg)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) {
            const int row = int(it.get_group(0));
            const int lid = int(it.get_local_id(0));
...
void run_shape(sycl::queue &q, int m, int k, int warmup, int iters, int *rc) {
    ...
    std::printf("two_launch_wg256,%d,%d,2,%.3f,%d,%d\n", m, k, two_us, max_abs_t,
                ok_t);
    std::printf("fused_wg256,%d,%d,1,%.3f,%d,%d\n", m, k, one_us, max_abs_f,
                ok_f);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--iters")
            ...
            std::fprintf(stderr,
                         "rmsnorm_epilogue_bw [--warmup W] [--iters N]\n");
            return 0;
        } else {
            std::fprintf(stderr, "rmsnorm_epilogue_bw: unknown arg %s\n",
                         a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "contract=rmsnorm_gamma1_eps1e-6_sym_s8_qmax127 wg=%d "
                "warmup=%d iters=%d\n",
                backend, name.c_str(), driver.c_str(), kWg, warmup, iters);
