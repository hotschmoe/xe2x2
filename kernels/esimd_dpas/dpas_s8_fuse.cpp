// K5+K2: RMSNorm-quant of f16 A fused into the M=1 s8 GEMM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: GEMM-only f16 is 34 us. K5 WG-256 RMSNorm-quant is
// 7-36 us extra launch. Steal: one launch, each WG redundantly
// RMSNorms A (tiny vs B), quant to s8, 64 dpas.8x4, *a_s*b_s, f16.
// Rank vs 34 us GEMM and vs two-launch ~41 us. Pad M=1 to RC=4.
...
struct DpasS8FuseNt2Name {};
struct DpasS8FuseNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_fuse: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
            ...
            for (int kk = 0; kk < dk; kk += kKc) {
                const esimd::simd<sycl::half, kRc * kKc> xf =
                    xesimd::lsc_load_2d<sycl::half, kKc, kRc>(
                        xd, unsigned(dk - 1), unsigned(rows - 1),
                        unsigned(dk - 1), kk, row0);
#pragma unroll
                for (int r = 0; r < kRc; ++r) {
#pragma unroll
                    for (int c = 0; c < kKc; ++c) {
                        const float v = float(xf[r * kKc + c]);
                        ss[r] += v * v;
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    esimd::simd<int8_t, kRc * kKc> a0, a1;
#pragma unroll
                    for (int r = 0; r < kRc; ++r) {
                        const float den = amx[r] > 0.f ? amx[r] : 1.f;
#pragma unroll
                        for (int c = 0; c < kKc; ++c) {
                            float q0 = float(x0[r * kKc + c]) * kQmax / den;
                            float q1 = float(x1[r * kKc + c]) * kQmax / den;
                            q0 = q0 >= 0.f ? float(int(q0 + 0.5f))
                                           : float(int(q0 - 0.5f));
                            q1 = q1 >= 0.f ? float(int(q1 + 0.5f))
                                           : float(int(q1 - 0.5f));
                            if (q0 > kQmax)
                                q0 = kQmax;
                            if (q0 < -kQmax)
                            ...
                            if (q1 < -kQmax)
                                q1 = -kQmax;
                            a0[r * kKc + c] = int8_t(q0);
                            a1[r * kKc + c] = int8_t(q1);
                        }
                    }
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_fuse: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8FuseNt4Name, 4, 8>(q, xd, bd, bsd, cd, rows, n,
                                                   k);
        return launch<DpasS8FuseNt2Name, 2, 16>(q, xd, bd, bsd, cd, rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_fuse --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_fuse: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_fuse: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "wg=%dx%d_alongN padM=RC b_scale=%.4f out=f16 "
