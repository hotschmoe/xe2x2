// K7 ESIMD GDN mixer T=64: packed conv1d K=4 then delta. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Packed q+k+v C=10240 conv over T, then 48-head delta with q/k repeat 16->48.
// S float in GRF. L2-norm q/k after conv (GDN). CONFIG prior: decode mixer
// 8.2, conv T=64 10.1, delta T=64 265. Napkin ~275. Rank pipe_host vs 275.
...
struct GdnMixerTConvName {};
struct GdnMixerTDeltaName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_mixer_t: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_conv(sycl::queue &q, const sycl::half *xd,
    ...
    return q.parallel_for<GdnMixerTConvName>(
...
sycl::event launch_delta(sycl::queue &q, const sycl::half *sd,
                         ...
                         int tlen) {
    return q.parallel_for<GdnMixerTDeltaName>(
...
#pragma unroll
            ...
            for (int t = 0; t < tlen; ++t) {
                ...
                const size_t vbase = ybase + size_t(kCq + kCk) +
                                     size_t(h) * size_t(kDv);
...
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    ...
                    qf[tile] = esimd::convert<float>(qv);
                    kf[tile] = esimd::convert<float>(kv);
                    qss += hsum16(qf[tile] * qf[tile]);
                    kss += hsum16(kf[tile] * kf[tile]);
                }
...
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    qf[tile] = qf[tile] * qinv;
                    kf[tile] = kf[tile] * kinv;
                    vold += hsum16(sf[tile] * kf[tile]);
                }
...
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    const esimd::simd<float, kVl> snew =
                        ah * (sf[tile] - bh * vold * kf[tile]) +
                        bh * vi * kf[tile];
                    sf[tile] = snew;
                    oacc += hsum16(snew * qf[tile]);
                }
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
               int iters, int *rc) {
    if (tlen < 1) {
        std::fprintf(stderr, "gdn_mixer_t: T=%d\n", tlen);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_mixer_t [--t 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_mixer_t: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_mixer_t conv+delta dtype=f16 T=%d C=%d nv=%d "
