// K7 ESIMD GDN fused-recurrent delta T=256 tile-fused pack a/b/v. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S. slmht leftover 260. Scalar a/b/v become SLM a/b plus
// 16-wide v along Dv. Rank pipe_host vs 260.
...
constexpr int kOffA = kOffQ + kBlk * kDk * int(sizeof(sycl::half));
constexpr int kOffB = kOffA + kBlk * int(sizeof(sycl::half));
constexpr int kOffV = kOffB + kBlk * int(sizeof(sycl::half));
constexpr int kSlmBytes = kOffV + kBlk * kWg * int(sizeof(sycl::half));

struct GdnDeltaSlmhtpName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_slmhtp: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaSlmhtpName>(
...
#pragma unroll
                ...
                {
                    const size_t abase =
                        size_t(t0 + lid) * size_t(kNv) + size_t(h);
                    esimd::simd<sycl::half, 1> ahv, bhv;
                    ahv.copy_from(ad + abase);
                    bhv.copy_from(bd + abase);
                    esimd::slm_block_store<sycl::half, 1>(
                        kOffA + lid * int(sizeof(sycl::half)), ahv);
                    esimd::slm_block_store<sycl::half, 1>(
                        kOffB + lid * int(sizeof(sycl::half)), bhv);
                    const int i0 = i - lid;
                    const size_t vbase =
                        (size_t(t0 + lid) * size_t(kNv) + size_t(h)) *
                        size_t(kDv);
                    esimd::simd<sycl::half, kWg> vv;
                    vv.copy_from(vd + vbase + size_t(i0));
                    esimd::slm_block_store<sycl::half, kWg>(
                        kOffV + lid * kWg * int(sizeof(sycl::half)), vv);
                }
...
#pragma unroll
                    ...
                    esimd::simd<sycl::half, 1> ahv, bhv;
                    ahv = esimd::slm_block_load<sycl::half, 1>(
                        kOffA + tt * int(sizeof(sycl::half)));
                    bhv = esimd::slm_block_load<sycl::half, 1>(
                        kOffB + tt * int(sizeof(sycl::half)));
                    esimd::simd<sycl::half, kWg> vv;
                    vv = esimd::slm_block_load<sycl::half, kWg>(
                        kOffV + tt * kWg * int(sizeof(sycl::half)));
                    const float ah = float(sycl::half(ahv[0]));
                    const float bh = float(sycl::half(bhv[0]));
                    const float vi = float(sycl::half(vv[lid]));
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_slmhtp: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_slmhtp [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_slmhtp: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_slmhtp dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d blk=%d "
