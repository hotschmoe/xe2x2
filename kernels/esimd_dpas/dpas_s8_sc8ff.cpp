// K2 ESIMD s8 RC=8, A-db + ngen null-dest prefetch, f16, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: sc8db A-db is 97-100 us vs W8A8 46. B-db+ca.ca (ax)
// was a GRF tax. ngen M=64 issues load_block2d to null (ff) then
// real loads. Steal: keep 8-row A ping-pong, add lsc_prefetch_2d
// cached/cached of next k64 A and B. No extra B GRF, no SLM.
// Rank pipe_host vs 97-100 and 46.
...
struct DpasS8Sc8ffNt2Name {};
struct DpasS8Sc8ffNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8ff: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
                ...
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    if (more_u) {
                        a0n = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                            ad, unsigned(dk - 1), unsigned(rows - 1),
                            unsigned(dk - 1), k0 + kK64, row0);
                        a1n = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                            ad, unsigned(dk - 1), unsigned(rows - 1),
                            unsigned(dk - 1), k0 + kK64 + kKc, row0);
                    } else if (o + 1 < outer) {
                        const int kn = (o + 1) * kInnerK;
                        a0n = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                            ad, unsigned(dk - 1), unsigned(rows - 1),
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8ff: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8ffNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS8Sc8ffNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8ff --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8ff: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8ff: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "wg=%dx%d_alongN A_double_buffer=1 prefetch=lsc_prefetch_2d "
                "ff_cached no_slm=1 "
