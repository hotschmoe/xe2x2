// K7 leftover: ESIMD s8 4-acc + wg 8x4 k128, f16, packed-qkv prefill.
...
// Square 4-acc 4x8 is 128 vs 75. Packed n=10240 is wide-N. Steal the
// same 4-acc k128 tile with wg 8 along N / 4 along M (N-heavy). Same
// thread count as 4x8. Rank pipe_host vs 274 and 164.
...
constexpr int kWgY = 4;
constexpr int kWgN = kWgX;
constexpr float kScale = 0.02f;

struct DpasS8Sc8w84m4Nt2Name {};
struct DpasS8Sc8w84m4Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w84m4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w84m4: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8w84m4Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                       rows, n, k);
        return launch<DpasS8Sc8w84m4Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                   n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w84m4 --nt 2|4 [--m 256] [--n 10240]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w84m4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8w84m4: --nt must be 2 or 4\n");
        return 2;
    }
