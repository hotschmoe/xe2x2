// K2 ESIMD s2 RC=8, A-db, ngen-style wg 4x8, f16, M=64, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s2 decode RC=4 is 11.5 us vs s4 16.5. s4 4x8 A-db is
// 33.6 vs W8A8 46. Steal native s2 onto that 4x8 tile (packed 4/byte
// along K, Transformed B). IGC s2 range [-2, 1]. Never E2M1 bitcast.
// Rank pipe_host vs s4 33.6 and W8A8 46.
...
struct DpasS2Db48Nt2Name {};
struct DpasS2Db48Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s2_db48: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
                        ...
                        acc[t] = xmx::dpas<8, kRc, int32_t, int32_t, uint8_t,
                                           uint8_t, xmx::dpas_argument_type::s2,
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s2_db48: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS2Db48Nt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                   rows, n, k);
        return launch<DpasS2Db48Nt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s2_db48 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s2_db48: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s2_db48: --nt must be 2 or 4\n");
        return 2;
    }
