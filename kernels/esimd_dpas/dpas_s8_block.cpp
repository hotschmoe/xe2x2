// K2 blocked s8 DPAS: reuse A across NT N-tiles (exec N=16 each).
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx -fsycl.
//
// CONFIG prior: 8x16 tile lost to oneDNN W8A8 at M=64 (274-373 vs 46-49 us)
// because each thread reloaded A for every 16 N. NT=2 -> 8x32, NT=4 -> 8x64.
// B still lsc_load_2d Transformed=true. Stream K; do not hold all K in GRF.
// CSV: phase,nt,m,n,k,us,TOPS,max_abs,ok
...
struct DpasS8N32Name {};
struct DpasS8N64Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_block: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
                ...
                for (int t = 0; t < NT; ++t) {
                    const esimd::simd<int8_t, kKc * kExecN> bt =
                        xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1, false,
                            ...
                            unsigned(cols - 1), col0 + t * kExecN, kc * kKc);
                    acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t, int8_t,
                                              int8_t>(acc[t], bt, afrag);
                }
            }
#pragma unroll
            for (int t = 0; t < NT; ++t)
...
void run_shape(sycl::queue &q, int nt, const char *phase, int m, int n, int k,
               int warmup, int iters, int *rc) {
    const int tn = nt * kExecN;
    if (m % kRc != 0 || n % tn != 0 || k % kKc != 0) {
        std::fprintf(stderr, "dpas_s8_block: shape m=%d n=%d k=%d nt=%d\n", m, n,
                     k, nt);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8N64Name, 4>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8N32Name, 2>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_block --nt 2|4 [--m 64] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_block: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_block: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                "RC=%d K=%d execN=%d NT=%d tileN=%d Bload=lsc_load_2d_Transformed "
                "warmup=%d iters=%d\n",
                backend, name.c_str(), driver.c_str(), kRc, kKc, kExecN, nt,
                nt * kExecN, warmup, iters);
    std::printf("phase,nt,m,n,k,us,TOPS,max_abs,ok\n");
    int rc = 0;
    const int check_n = nt * kExecN;
    run_shape(q, nt, "check", kRc, check_n, kKc, warmup, iters, &rc);
    run_shape(q, nt, "timed", timed_m, timed_n, timed_k, warmup, iters, &rc);
    return rc;
}
