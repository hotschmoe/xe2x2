// K2 ESIMD s8 RC=4, k64 unroll, NT N-tiles. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx -fsycl (intel/llvm#21741).
//
// CONFIG prior: W8A8 M=1 ngen is 64x dpas.8x4, k64, N-tile 64.
// Inner body: 2 K-chunks x NT B panels, A reused across NT, no SLM.
// NT=2 -> 4 dpas/step; NT=4 -> 8 dpas/step. CSV: phase,nt,m,n,k,us,TOPS,max_abs,ok
...
struct DpasS8K64Nt2Name {};
struct DpasS8K64Nt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_k64: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
                ...
                for (int t = 0; t < NT; ++t) {
                    ...
                    const esimd::simd<int8_t, kKc * kExecN> b1 =
                        xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1, false,
                                            true>(
                            bd, unsigned(cols - 1), unsigned(dk - 1),
                            unsigned(cols - 1), col0 + t * kExecN, k0 + kKc);
                    acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t, int8_t,
                                              int8_t>(acc[t], b1, a1);
                }
            }
...
void run_shape(sycl::queue &q, int nt, const char *phase, int m, int n, int k,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % kK64 != 0) {
        std::fprintf(stderr, "dpas_s8_k64: shape m=%d n=%d k=%d nt=%d\n", m, n,
                     k, nt);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8K64Nt4Name, 4>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8K64Nt2Name, 2>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_k64 --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_k64: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_k64: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                "RC=%d K64=%d execN=%d NT=%d tileN=%d unroll=2k*NT "
    ...
    int rc = 0;
    run_shape(q, nt, "check", kRc, nt * kExecN, kK64, warmup, iters, &rc);
