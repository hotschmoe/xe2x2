// K2 ESIMD s8 RC=4, 64 dpas.8x4 + prefetch overlapped with dpas.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: ngen M=1 is load A/B, then ff (null-dest prefetch of next),
// then dpas.8x4. dpas_s8_pf issued ff *before* the current loads and taxed
// decode (83-208 vs u64 53-69 vs W8A8 45). Steal ngen order: load current,
// first dpas, then lsc_prefetch_2d of next k64 so the send is in-flight
// during the remaining dpas. Prologue prefetches k=0. Surface-bounded
// prefetch, no kn<dk jmpi in front of loads.
...
struct DpasS8OvNt2Name {};
struct DpasS8OvNt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_ov: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_ov: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8OvNt4Name, 4, 8>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8OvNt2Name, 2, 16>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_ov --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_ov: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_ov: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d prefetch=lsc_prefetch_2d_overlap Bload=Transformed "
                "warmup=%d iters=%d\n",
