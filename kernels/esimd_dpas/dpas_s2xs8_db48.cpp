// K2 ESIMD s2xs8 RC=8, A-db, wg 4x8, f16, M=64, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Mix: A=s8 B=s2 pack=4 K=32 dpas (OPC=4). Same 4x8 A-db as s2 20 us.
// Decode s2xs8 is 14.1 vs s2 11.5 vs s8 34. IGC s2 [-2,1]. Never E2M1.
// Rank pipe_host vs s2 20, s8xs4 43.3, s8 75, W8A8 46.
...
struct DpasS2xS8Db48Nt2Name {};
struct DpasS2xS8Db48Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s2xs8_db48: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
                        ...
                        acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t,
                                                  uint8_t, int8_t,
                                                  esimd::xmx::dpas_argument_type::s2,
                                                  esimd::xmx::dpas_argument_type::s8>(
                            acc[t], b0, a0);
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s2xs8_db48: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS2xS8Db48Nt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                      rows, n, k);
        return launch<DpasS2xS8Db48Nt2Name, 2, 16>(q, ad, bd, asd, bsd, cd,
                                                   rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s2xs8_db48 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s2xs8_db48: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s2xs8_db48: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s2xs8->f16_scaled RC=%d NT=%d unroll=%d dpas=%d "
