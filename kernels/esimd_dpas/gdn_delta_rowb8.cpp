// K7 ESIMD GDN fused-recurrent delta T=256 row-block rb=8. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S. 8 rows per WI, one WG per head. rb=4 was 1034 vs SLM-K 847.
// Rank pipe_host vs 1034 and 847.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_rowb8: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaRowb8Name>(
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < 1) {
        std::fprintf(stderr, "gdn_delta_rowb8: T=%d\n", tlen);
        *rc = 2;
        return;
    }
    const size_t ns = size_t(kNv) * size_t(kDv) * size_t(kDk);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_rowb8 [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_rowb8: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_rowb8 dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d rb=%d "
