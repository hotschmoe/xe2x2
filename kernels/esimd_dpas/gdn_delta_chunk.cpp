// K7 ESIMD GDN chunk/WY delta prefill T=256 C=16. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// FLA chunk_gated_delta_rule C=16: KK^T, (I+L)^{-1}, w/u, h+o.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_chunk: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaChunkName>(
...
#pragma unroll
                ...
                }
                {
                    ...
                    esimd::slm_block_store<float, 1>(
                        kOffAl + lid * int(sizeof(float)), af);
                    esimd::slm_block_store<float, 1>(
                        kOffB + lid * int(sizeof(float)), bf);
                }
...
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        const int kt =
                            kOffK + (lid * kDk + tile * kVl) * int(sizeof(sycl::half));
                        const int ks =
                            kOffK + (s * kDk + tile * kVl) * int(sizeof(sycl::half));
                        ...
                        acc += hsum16(esimd::convert<float>(a) *
...
#pragma unroll
                for (int s = 0; s < kC; ++s) {
                    float lts = 0.f;
                    if (lid > s)
                        lts = beta[lid] * sycl::exp2(g[lid] - g[s]) * kkrow[s];
                    esimd::simd<float, 1> lv;
                    lv[0] = lts;
                    esimd::slm_block_store<float, 1>(
                        kOffA + (lid * kC + s) * int(sizeof(float)), lv);
                    esimd::simd<float, 1> qv;
                    qv[0] = qkrow[s];
                    esimd::slm_block_store<float, 1>(
                        kOffQK + (lid * kC + s) * int(sizeof(float)), qv);
                }
                it.barrier(sycl::access::fence_space::local_space);
                if (lid == 0) {
                    float L[kC][kC];
...
#pragma unroll
                        for (int s = 0; s < kC; ++s) {
                            esimd::simd<float, 1> lv;
                            lv = esimd::slm_block_load<float, 1>(
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kC || (tlen % kC) != 0) {
        std::fprintf(stderr, "gdn_delta_chunk: T=%d need multiple of %d\n", tlen,
                     kC);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_chunk [--t 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_chunk: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_chunk dtype=f16 T=%d C=%d nv=%d dv=%d dk=%d VL=%d "
