// K2 ESIMD DPAS s2 x s8 -> s32. Backend: sycl+l0. AOT intel_gpu_bmg_g31.
// Standalone icpx -fsycl binary (intel/llvm#21741: fat SYCL trees lie on B70).
//
// Literature mix (arXiv 2508.06753): dpas.8x8 rD:d rAcc:d rW:s2 rA:b, VNNI16.
// CONFIG prior from the paper (NOT a result): this mix has the SAME systolic
// rate as s8xs8 (OPS_PER_CHAN=4, K=32), not 4x. IGC s2 range is [-2, 1],
// two's complement, not E2M1. Measure later; this binary only needs to
// compile on this pass.
//
// Tile: RC=8, K=32 per dpas, exec N=16.
// B (s2 weights): packed 4 s2/byte along K (VNNI16 = 16 int2 / dword),
// then lsc_load_2d Transformed=true on that uint8 surface.
// A (s8 activations): row-major int8, plain lsc_load_2d.
// Never bitcast NVFP4 E2M1 onto s2.
//
...
constexpr int kPackB = 4; // s2 per uint8 along K

struct DpasS2xS8Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s2xs8: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}

// IGC s2: two's complement 2-bit, codes {0,1,-2,-1} == [-2, 1].
...
void pack_b_s2(const int8_t *b, uint8_t *out, int k, int n) {
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const uint8_t *bd,
    ...
    constexpr int kBPackedH = kKc / kPackB; // 8 rows of packed s2
    return q.parallel_for<DpasS2xS8Name>(
...
void run_shape(sycl::queue &q, const char *phase, int m, int n, int k,
    ...
    if (m % kRc != 0 || n % kExecN != 0 || k % kKc != 0) {
        std::fprintf(stderr, "dpas_s2xs8: shape m=%d n=%d k=%d not aligned\n", m,
                     n, k);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--check-m")
            ...
            std::fprintf(stderr,
                         "dpas_s2xs8 [--check-m 8] [--check-n 16] [--check-k 32] "
                         "[--m 1024] [--n 512] [--k 1024] [--iters N] [--warmup W]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s2xs8: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    const char *backend = q.get_backend() == sycl::backend::ext_oneapi_level_zero
                              ? "sycl+l0"
                              : "sycl+other";
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s2xs8s32 "
                "RC=%d K=%d N=%d OPC=4 paper=2508.06753 s2_range=[-2,1] "
                "Bload=lsc_load_2d_Transformed warmup=%d iters=%d "
                "note=same_rate_as_s8xs8_prior_not_result\n",
