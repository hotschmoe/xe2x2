// K2 ESIMD s8 RC=8, 64 dpas.8x8, A double-buffer, f16 scale, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: sc8 M=64 is 120 us vs W8A8 46. sc84 4x2x4+SLM is 136
// (barrier-per-k64 tax). ngen M=64 is wg 4x2x4 kr xaf k64: software
// pipeline of A/B, not SLM share. Steal ska-style A ping-pong on the
// sc8 tile: prologue A[k=0], issue A[k+64] before dpas of current.
// No SLM, no barrier. Rank pipe_host vs 120 and 46.
...
struct DpasS8Sc8dbNt2Name {};
struct DpasS8Sc8dbNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8db: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8db: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8dbNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS8Sc8dbNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8db --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8db: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8db: --nt must be 2 or 4\n");
        return 2;
    }
