// K2 ESIMD s8 RC=4, 64 dpas.8x4, ngen wg 8x2 2D launch.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: ngen M=1 is wg 8x2 wx4 k64. Hand u64 used 1D local=16
// (M-major). K-pipe (ff/ska) lost to that 1D tile. Steal launch only:
// nd_range<2> local {8,2} = (N, M), same per-thread NT tile, no SLM,
// no extra ff. Idle lanes when m_blocks % 2 != 0 (M=4 pads to 2).
...
constexpr int kWgN = 8;
constexpr int kWgM = 2;

struct DpasS8WgNt2Name {};
struct DpasS8WgNt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_wg: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    const size_t g0 = round_up(n_groups, kWgN);
    const size_t g1 = round_up(m_blocks, kWgM);
    return q.parallel_for<Name>(
        sycl::nd_range<2>({g0, g1}, {size_t(kWgN), size_t(kWgM)}),
        [=](sycl::nd_item<2> it) SYCL_ESIMD_KERNEL {
            const int64_t ng = int64_t(it.get_global_id(0));
            const int64_t mb = int64_t(it.get_global_id(1));
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_wg: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8WgNt4Name, 4, 8>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8WgNt2Name, 2, 16>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_wg --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_wg: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_wg: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d wg=%dx%d Bload=Transformed warmup=%d iters=%d\n",
                backend, name.c_str(), driver.c_str(), kRc, kK64, kExecN, nt,
                nt * kExecN, unroll, 2 * nt * unroll, unroll * kK64, kWgN, kWgM,
                warmup, iters);
