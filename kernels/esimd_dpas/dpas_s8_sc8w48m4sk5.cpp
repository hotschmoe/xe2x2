// K7 leftover: ESIMD s8 4-acc wg 4x8 k128 split-K=5, f16, packed qkv.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: SK=2 unroll=5 M=256 is 295 vs W8A8 164 vs 4-acc 274.
// Occupancy win at M=64 died at M=256 (unroll tax + reduce). Steal
// SK=5 with original unroll=8: each WI one k1024 outer, 5x K occupancy.
// K=5120 / 5 = 1024. Rank pipe_host vs 274 and 164.
...
struct DpasS8Sc8w48m4sk5Nt2Name {};
struct DpasS8Sk5ReduceName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4sk5: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_reduce(sycl::queue &q, const float *cpart, const float *asd,
    ...
    return q.parallel_for<DpasS8Sk5ReduceName>(
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % (inner_k * kSK) != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m4sk5: shape m=%d n=%d k=%d nt=%d unroll=%d sk=%d\n",
                     m, n, k, nt, unroll, kSK);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        (void)launch_sk<DpasS8Sc8w48m4sk5Nt2Name, 2, 8>(q, ad, bd, cpart, rows,
                                                        n, k);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m4sk5 --nt 2 [--m 256] [--n 10240]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m4sk5: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4sk5: --nt must be 2\n");
        return 2;
    }
    const int unroll = 8;
