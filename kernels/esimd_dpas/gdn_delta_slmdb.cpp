// K7 ESIMD GDN fused-recurrent delta T=256 SLM double-buffer. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S. Ping-pong two k/q SLM buffers, one barrier per blk.
// CONFIG prior: SLM-K 847 leftover. Rank pipe_host vs 847.
...
constexpr int kHalf = int(sizeof(sycl::half));
constexpr int kBuf = kBlk * kDk * kHalf;
constexpr int kOffK0 = 0;
constexpr int kOffQ0 = kBuf;
constexpr int kOffK1 = 2 * kBuf;
constexpr int kOffQ1 = 3 * kBuf;
constexpr int kSlmBytes = 4 * kBuf;

struct GdnDeltaSlmdbName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_slmdb: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaSlmdbName>(
...
#pragma unroll
            ...
            {
                const size_t kbase =
                    (size_t(lid) * size_t(kNv) + size_t(h)) * size_t(kDk);
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    ...
                    esimd::slm_block_store<sycl::half, kVl>(
                        kOffK0 + (lid * kDk + tile * kVl) * kHalf, kv);
                    esimd::slm_block_store<sycl::half, kVl>(
                        kOffQ0 + (lid * kDk + tile * kVl) * kHalf, qv);
                }
            }
            it.barrier(sycl::access::fence_space::local_space);
            for (int b = 0; b < nblk; ++b) {
                const int t0 = b * kBlk;
                const int offK = (b & 1) ? kOffK1 : kOffK0;
                const int offQ = (b & 1) ? kOffQ1 : kOffQ0;
                if (b + 1 < nblk) {
                    const int t1 = (b + 1) * kBlk;
                    const int nK = (b & 1) ? kOffK0 : kOffK1;
                    const int nQ = (b & 1) ? kOffQ0 : kOffQ1;
                    const size_t kbase =
                        (size_t(t1 + lid) * size_t(kNv) + size_t(h)) *
                        size_t(kDk);
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        esimd::simd<sycl::half, kVl> kv, qv;
                        ...
                        qv.copy_from(qd + kbase + size_t(tile * kVl));
                        esimd::slm_block_store<sycl::half, kVl>(
                            nK + (lid * kDk + tile * kVl) * kHalf, kv);
                        esimd::slm_block_store<sycl::half, kVl>(
                            nQ + (lid * kDk + tile * kVl) * kHalf, qv);
                    }
                }
...
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        kv[tile] = esimd::slm_block_load<sycl::half, kVl>(
                            offK + (tt * kDk + tile * kVl) * kHalf);
                        qv[tile] = esimd::slm_block_load<sycl::half, kVl>(
                            offQ + (tt * kDk + tile * kVl) * kHalf);
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_slmdb: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_slmdb [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_slmdb: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_slmdb dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d blk=%d "
