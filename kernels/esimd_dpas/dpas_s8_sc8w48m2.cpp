// K7 leftover: ESIMD s8 2-acc M-tile + wg 4x8 k128, f16, packed qkv M=256.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: 4-acc packed M=256 is 274 vs W8A8 164. SK=2 295 SK=5
// 393 lost (occupancy tax at large M). Steal 2-acc: 16 rows/thread,
// 2x M WGs vs 4-acc. Same k128 wg 4x8. Rank pipe_host vs 274 and 164.
...
struct DpasS8Sc8w48m2Nt2Name {};
struct DpasS8Sc8w48m2Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m2: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m2: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8w48m2Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                       rows, n, k);
        return launch<DpasS8Sc8w48m2Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                   n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m2 --nt 2|4 [--m 256] [--n 10240]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m2: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8w48m2: --nt must be 2 or 4\n");
        return 2;
    }
