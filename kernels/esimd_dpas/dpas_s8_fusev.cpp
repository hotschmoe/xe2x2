// K5+K2: vectorized RMSNorm-quant of f16 A fused into the M=1 s8 GEMM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: scalar dpas_s8_fuse was 313 us, cosine 0.73. Causes:
// f16 lsc_load_2d width/pitch passed as elements (bytes required), so
// K>~surface/2 was OOB; scalar rint/sqrt in the k-loop (K6-class tax).
// Steal: simd convert/reduce/hmax/rnde, pitch in bytes, 64 dpas.8x4,
// *a_s*b_s, f16. Rank vs 34 us GEMM and two-launch ~41-70. Pad M=1 to RC=4.
...
struct DpasS8FusevNt2Name {};
struct DpasS8FusevNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_fusev: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
inline void accum_stats(esimd::simd<sycl::half, kRc * kKc> xf, float ss[kRc],
                        float amx[kRc]) {
    esimd::simd<float, kRc * kKc> vf = esimd::convert<float>(xf);
#pragma unroll
    for (int r = 0; r < kRc; ++r) {
        esimd::simd<float, kKc> row = vf.template select<kKc, 1>(r * kKc);
        ss[r] += esimd::reduce<float>(row * row, std::plus<>{});
        esimd::simd<float, kKc> ar = row;
        ar.merge(-row, row < 0.f);
        const float mx = esimd::hmax<float>(ar);
        if (mx > amx[r])
            amx[r] = mx;
    }
}

inline esimd::simd<int8_t, kRc * kKc>
quant_tile(esimd::simd<sycl::half, kRc * kKc> xf, const float invs[kRc],
           const float asv_s[kRc]) {
    esimd::simd<float, kRc * kKc> vf = esimd::convert<float>(xf);
    esimd::simd<float, kRc * kKc> q(0.f);
#pragma unroll
    for (int r = 0; r < kRc; ++r) {
        esimd::simd<float, kKc> row = vf.template select<kKc, 1>(r * kKc);
        const float den = asv_s[r] > 0.f ? asv_s[r] : 1.f;
        q.template select<kKc, 1>(r * kKc) = row * (invs[r] / den);
    }
    q = esimd::rnde<float, kRc * kKc>(q);
    q.merge(esimd::simd<float, kRc * kKc>(kQmax), q > kQmax);
    q.merge(esimd::simd<float, kRc * kKc>(-kQmax), q < -kQmax);
    return esimd::convert<int8_t>(q);
}

template <typename Name, int NT, int kUnroll>
sycl::event launch(sycl::queue &q, const sycl::half *xd, const int8_t *bd,
                   const float *bsd, sycl::half *cd, int rows, int cols,
                   int dk) {
    ...
    const unsigned x_w = unsigned(dk) * unsigned(sizeof(sycl::half)) - 1u;
    const unsigned x_h = unsigned(rows - 1);
...
#pragma unroll
            ...
            for (int kk = 0; kk < dk; kk += kKc) {
                const esimd::simd<sycl::half, kRc * kKc> xf =
                    xesimd::lsc_load_2d<sycl::half, kKc, kRc>(xd, x_w, x_h, x_w,
                                                              kk, row0);
                accum_stats(xf, ss, amx);
            }
            float asv_s[kRc];
            float invs[kRc];
#pragma unroll
            for (int r = 0; r < kRc; ++r) {
                invs[r] = 1.f / __builtin_sqrtf(ss[r] / float(dk) + kEps);
                float s = amx[r] * invs[r] / kQmax;
                if (s <= 0.f)
                    s = 1.f;
                asv_s[r] = s;
            }
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    const esimd::simd<sycl::half, kRc * kKc> x0 =
                        xesimd::lsc_load_2d<sycl::half, kKc, kRc>(xd, x_w, x_h,
                                                                  x_w, k0,
                                                                  row0);
                    const esimd::simd<sycl::half, kRc * kKc> x1 =
                        xesimd::lsc_load_2d<sycl::half, kKc, kRc>(
                            xd, x_w, x_h, x_w, k0 + kKc, row0);
                    const esimd::simd<int8_t, kRc * kKc> a0 =
                        quant_tile(x0, invs, asv_s);
                    const esimd::simd<int8_t, kRc * kKc> a1 =
                        quant_tile(x1, invs, asv_s);
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_fusev: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    for (int i = 0; i < m; ++i) {
        ...
        float as = amx * inv / kQmax;
        if (as <= 0.f)
            as = 1.f;
        const float den = amx > 0.f ? amx : 1.f;
        for (int j = 0; j < n; ++j) {
            float acc = 0.f;
            for (int kk = 0; kk < k; ++kk) {
                float q = float(hx[size_t(i) * size_t(k) + size_t(kk)]) *
                          kQmax / den;
                q = std::nearbyintf(q);
                if (q > kQmax)
                    q = kQmax;
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8FusevNt4Name, 4, 8>(q, xd, bd, bsd, cd, rows, n,
                                                    k);
        return launch<DpasS8FusevNt2Name, 2, 16>(q, xd, bd, bsd, cd, rows, n,
                                                 k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_fusev --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_fusev: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_fusev: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=f16A_rmsnorm_s8GEMM_f16 RC=%d NT=%d unroll=%d dpas=%d "
                "wg=%dx%d_alongN padM=RC b_scale=%.4f out=f16 simd_quant=1 "
                "f16_pitch=bytes warmup=%d iters=%d card=%d spin=%d heat=none\n",
