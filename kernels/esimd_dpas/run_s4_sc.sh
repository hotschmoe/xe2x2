...
OUT="$ROOT/results/k2/s4sc_n${NT}_s${SPIN}_card${CARD}.txt"
FREQ="$ROOT/results/k2/s4sc_n${NT}_s${SPIN}_card${CARD}.freq"
...
trap cleanup EXIT
{
  ...
  echo "CONFIG backend=sycl+l0 card=$CARD arm=dpas_s4_sc nt=$NT spin=$SPIN heat=none out=f16 pack=s4"
  echo "s8 scale-to-f16 M=1 34 us. W8A8 M=1 44 us."
