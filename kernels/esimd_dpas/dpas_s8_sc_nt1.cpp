// K7 leftover: ESIMD s8 RC=4 NT=1, 8x2 along N, scale-to-f16.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: o-proj M=1 n=5120 k=6144 sc NT=2 is 62 us vs W8A8 47.
// NT=4 is 103. Square NT=2 is 34. Napkin K-linear ~41. NT=2 has 10 WGs
// at n=5120 (kTN=32). NT=1 kTN=16 -> 20 WGs. Occupancy steal, not
// another NT=2/4 shelf fire. Rank pipe_host vs 62 and 47.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc_nt1: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc_nt1: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        return launch<DpasS8ScNt1Name, 1, 16>(q, ad, bd, asd, bsd, cd, rows, n,
                                              k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc_nt1 --nt 1 [--m 1] [--n 5120] [--k 6144]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc_nt1: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 1) {
        std::fprintf(stderr, "dpas_s8_sc_nt1: --nt must be 1\n");
        return 2;
    }
    const int unroll = 16;
