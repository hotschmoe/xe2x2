// K2 ESIMD s8 RC=8, ngen wg 4x2x4 + SLM A pack, f16 scale epilogue.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: RC=8 8x2-N (sc8) is 120 us vs W8A8 M=64 46. ngen is
// wg 4x2x4 kr grf256 k64 + 53 SLM, 64x dpas.8x8. Steal the WG and
// A-share: 32-thread 4x2x4, 4x RC=8 (32 rows/thread), SLM A per k64,
// NT=2 (32 N/thread). GRF256 requested. Rank pipe_host vs 46 and 120.
...
constexpr int kSlmHalf = kRowsTh * kK64;
constexpr int kSlmBytes = kWgY * kSlmHalf;
constexpr float kScale = 0.02f;

struct DpasS8Sc84Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc84: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    return q.parallel_for<DpasS8Sc84Name>(
        sycl::nd_range<3>({g0, g1, g2}, {size_t(kWgX), size_t(kWgY), size_t(kWgZ)}),
        props, [=](sycl::nd_item<3> it) SYCL_ESIMD_KERNEL {
            esimd::slm_init<kSlmBytes>();
            const int lid0 = int(it.get_local_id(0));
            const int lid1 = int(it.get_local_id(1));
            const int lid2 = int(it.get_local_id(2));
            const int lid_n = lid2 * kWgX + lid0;
            const int64_t ng = int64_t(it.get_group(0)) * kWgN + int64_t(lid_n);
            const int64_t mb = int64_t(it.get_group(1)) * kWgY + int64_t(lid1);
            const bool live = (ng < n_groups && mb < m_blocks);
            const int row0 = int(mb * kRowsTh);
            const int col0 = int(ng * kTN);
            const unsigned slm_base = unsigned(lid1 * kSlmHalf);
                        ...
                        for (int r = 0; r < kMTiles; ++r) {
                            const esimd::simd<int8_t, kRc * kKc> a0 =
                                xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                    ...
                                    unsigned(dk - 1), k0, row0 + r * kRc);
                            const esimd::simd<int8_t, kRc * kKc> a1 =
                                xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                    ad, unsigned(dk - 1), unsigned(rows - 1),
                                    unsigned(dk - 1), k0 + kKc, row0 + r * kRc);
                            esimd::slm_block_store<int8_t, kRc * kKc>(
                                slm_base + unsigned(r * kRc * kKc), a0);
                            esimd::slm_block_store<int8_t, kRc * kKc>(
                                slm_base + unsigned(kMTiles * kRc * kKc +
                                                    r * kRc * kKc),
                                a1);
                        }
                    }
...
#pragma unroll
                        for (int r = 0; r < kMTiles; ++r) {
                            a0[r] = esimd::slm_block_load<int8_t, kRc * kKc>(
                                slm_base + unsigned(r * kRc * kKc));
                            a1[r] = esimd::slm_block_load<int8_t, kRc * kKc>(
                                slm_base + unsigned(kMTiles * kRc * kKc +
                                                    r * kRc * kKc));
                        }
#pragma unroll
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc84: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    const int rows = int(round_up(m, kRowsTh * kWgY));
    ...
    auto go = [&]() -> sycl::event {
        return launch<2, 4>(q, ad, bd, asd, bsd, cd, rows, n, k);
    };
    (void)nt;
    (void)unroll;
    ...
    const double tops = (ops / 1.e12) / (us * 1.e-6);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc84 --nt 2 [--m 64] [--spin 512]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc84: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2) {
        std::fprintf(stderr, "dpas_s8_sc84: --nt must be 2\n");
        return 2;
    }
    const int unroll = 4;
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s8xs8->f16_scaled RC=%d Mtiles=%d NT=%d unroll=%d "
                "dpas=%d wg=%dx%dx%d slm=%d grf256_request=1 "
                "a_scale=b_scale=%.4f out=f16 warmup=%d iters=%d card=%d "
                "spin=%d heat=none\n",
                backend, name.c_str(), driver.c_str(), kRc, kMTiles, nt, unroll,
                kMTiles * 2 * nt * unroll, kWgX, kWgY, kWgZ, kSlmBytes,
                double(kScale), warmup, iters, g_card, g_spin);
    int rc = 0;
    run_shape(q, nt, unroll, "check", kRowsTh * kWgY, nt * kExecN,
