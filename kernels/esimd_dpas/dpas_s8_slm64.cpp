// K2 ESIMD s8 RC=4, 64 dpas.8x4, 8x2 along N, SLM A share per k64.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: ngen M=1 is wg 8x2 + SLM + 64 dpas.8x4. Old SLM
// (barrier per k32, no unroll) lost 372-463 vs 77-168. wgn 8x2-N
// is 47-50 us with each of 16 N-lanes reloading the same A. Steal
// the bundle: lid0 loads A[k64] to SLM, barrier, 16 lanes dpas.
// Two barriers per k64, not per k32. Padded N-lanes stay in barriers.
...
constexpr int kSlmA = kRc * kKc;

struct DpasS8Slm64Nt2Name {};
struct DpasS8Slm64Nt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_slm64: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    return q.parallel_for<Name>(
        ...
        [=](sycl::nd_item<2> it) SYCL_ESIMD_KERNEL {
            esimd::slm_init<256>();
            const int lid = int(it.get_local_id(1)) * kWgX +
                            int(it.get_local_id(0));
            const int64_t ng = int64_t(it.get_group(0)) * kWgN + int64_t(lid);
            const int64_t mb = int64_t(it.get_group(1));
            const bool live = (ng < n_groups);
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    if (live && lid == 0) {
                        const esimd::simd<int8_t, kSlmA> a0 =
                            xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                ad, unsigned(dk - 1), unsigned(rows - 1),
                                unsigned(dk - 1), k0, row0);
                        const esimd::simd<int8_t, kSlmA> a1 =
                            xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                ad, unsigned(dk - 1), unsigned(rows - 1),
                                unsigned(dk - 1), k0 + kKc, row0);
                        esimd::slm_block_store<int8_t, kSlmA>(0, a0);
                        esimd::slm_block_store<int8_t, kSlmA>(kSlmA, a1);
                    }
                    esimd::barrier();
                    if (live) {
                        const esimd::simd<int8_t, kSlmA> a0 =
                        ...
                        const esimd::simd<int8_t, kSlmA> a1 =
                            esimd::slm_block_load<int8_t, kSlmA>(kSlmA);
#pragma unroll
                        for (int t = 0; t < NT; ++t) {
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_slm64: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Slm64Nt4Name, 4, 8>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8Slm64Nt2Name, 2, 16>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_slm64 --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_slm64: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_slm64: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d wg=%dx%d_alongN slmA=k64 Bload=Transformed warmup=%d iters=%d\n",
