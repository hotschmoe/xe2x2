// K6 ESIMD s4 RC=4 GPTQ group-scale f16 decode. M=1 pad, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Same wg 8x2 as dpas_s4_sc. B is real GPTQ s4 + g128 f16 scales.
...
struct DpasS4GptqScNt2Name {};
struct DpasS4GptqScNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s4_gptq_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
