// K2 ESIMD s8 4-acc M-tile + wg 4x8 k128, f16, M=256, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: packed qkv M=256 4-acc wg 4x8 is 274 vs W8A8 164.
// Label: hail-mary persist/mainloop.
...
struct DpasS8Sc8w48m4bpNt2Name {};
struct DpasS8Sc8w48m4bpNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4bp: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
            ...
            esimd::simd<int8_t, kRc * kKc> a_cur[kMTiles];
            esimd::simd<int8_t, kRc * kKc> a_next[kMTiles];
            esimd::simd<int8_t, kKc * kExecN> b_cur[NT];
            esimd::simd<int8_t, kKc * kExecN> b_next[NT];
#pragma unroll
            for (int r = 0; r < kMTiles; ++r)
                a_cur[r] = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                    ad, unsigned(dk - 1), unsigned(rows - 1),
                    unsigned(dk - 1), 0, row0 + r * kRc);
#pragma unroll
            for (int t = 0; t < NT; ++t)
                b_cur[t] =
...
#pragma unroll
                            for (int t = 0; t < NT; ++t)
                                b_next[t] = xesimd::lsc_load_2d<
                                    int8_t, kExecN, kKc, 1, false, true>(
                                    bd, unsigned(cols - 1), unsigned(dk - 1),
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m4bp: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8w48m4bpNt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                       rows, n, k);
        return launch<DpasS8Sc8w48m4bpNt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                   n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m4bp --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m4bp: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4bp: --nt must be 2 or 4\n");
        return 2;
    }
