// K7 leftover / hail-mary persist: ESIMD s8 RC=4 NT=2, B software pipeline.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: o-proj sc NT=2 62 vs W8A8 47. ska A-pipeline lost decode
// vs u64. At M=1 B is the traffic (K*N), A is 4*K. Steal: prologue load
// k0 A/B, issue next k64 A/B into GRF before current dpas. Same 8x2 WG
// as sc. Label hail-mary persist/mainloop. Rank pipe_host vs 62 and 47.
...
struct DpasS8ScBpNt2Name {};
struct DpasS8ScBpNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc_bp: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
            for (int t = 0; t < NT; ++t)
                acc[t] = 0;
            const int nstep = dk / kK64;
            esimd::simd<int8_t, kRc * kKc> a0c, a1c, a0n, a1n;
            esimd::simd<int8_t, kKc * kExecN> b0c[NT], b1c[NT], b0n[NT],
                b1n[NT];
            a0c = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                ad, unsigned(dk - 1), unsigned(rows - 1), unsigned(dk - 1), 0,
                row0);
            a1c = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                ad, unsigned(dk - 1), unsigned(rows - 1), unsigned(dk - 1), kKc,
                row0);
#pragma unroll
            for (int t = 0; t < NT; ++t) {
                b0c[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1, false,
                                             true>(
                    bd, unsigned(cols - 1), unsigned(dk - 1),
                    unsigned(cols - 1), col0 + t * kExecN, 0);
                b1c[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1, false,
                                             true>(
                    bd, unsigned(cols - 1), unsigned(dk - 1),
                    unsigned(cols - 1), col0 + t * kExecN, kKc);
            }
            for (int s = 0; s < nstep; ++s) {
                const int kn = (s + 1) * kK64;
                if (s + 1 < nstep) {
                    a0n = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                        ad, unsigned(dk - 1), unsigned(rows - 1),
                        unsigned(dk - 1), kn, row0);
                    a1n = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                        ad, unsigned(dk - 1), unsigned(rows - 1),
                        unsigned(dk - 1), kn + kKc, row0);
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
                        ...
                        b1n[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1,
                                                     false, true>(
                            bd, unsigned(cols - 1), unsigned(dk - 1),
                            unsigned(cols - 1), col0 + t * kExecN, kn + kKc);
                    }
                }
#pragma unroll
                for (int t = 0; t < NT; ++t) {
                    acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t, int8_t,
                                              int8_t>(acc[t], b0c[t], a0c);
                    acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t, int8_t,
                                              int8_t>(acc[t], b1c[t], a1c);
                }
                if (s + 1 < nstep) {
                    a0c = a0n;
                    a1c = a1n;
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc_bp: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8ScBpNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                   rows, n, k);
        return launch<DpasS8ScBpNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc_bp --nt 2|4 [--m 1] [--n 5120] [--k 6144]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc_bp: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc_bp: --nt must be 2 or 4\n");
        return 2;
    }
