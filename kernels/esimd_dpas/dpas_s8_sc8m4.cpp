// K2 ESIMD s8 4-acc M-tile (32 rows/thread), no SLM, f16 scale.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: sc8db A-db is 97-100 us vs W8A8 46. sc84 4x RC=8
// with SLM is 136. ngen M=64 keeps 4 acc along M (32 rows) and
// pipelines A/B without sharing A through SLM. Steal: each thread
// owns 32 rows, 4x dpas.8x8 acc, loads its own A, reuses B across
// the 4 M-tiles. A ping-pong, no SLM, no barrier. NT=2 U=4 -> 64
// dpas. Rank pipe_host vs 97-100 and 46.
...
struct DpasS8Sc8m4Nt2Name {};
struct DpasS8Sc8m4Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8m4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8m4: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8m4Nt4Name, 4, 2>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS8Sc8m4Nt2Name, 2, 4>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8m4 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8m4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8m4: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "unroll=%d dpas=%d wg=%dx%d_alongN A_double_buffer=1 no_slm=1 "
                ...
                unroll, 2 * nt * unroll * kMTiles, kWgX, kWgY, double(kScale),
                warmup, iters, g_card, g_spin);
    int rc = 0;
    run_shape(q, nt, unroll, "check", kRowsTh, nt * kExecN, unroll * kK64, 1, 1,
              &rc, 0);
