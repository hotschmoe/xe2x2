// K2 ESIMD s2 RC=4, 8x2 along N, scale-to-f16, M=1 pad, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s8 RC=4 8x2-N is 34 us, s4 same tile is 16.5 us.
// Steal native s2 onto the decode tile: packed 4/byte along K,
// one s2 dpas per k64. Same wg 8x2 along N, pad M to RC=4.
// IGC s2 range is [-2, 1]. Never E2M1 bitcast. Rank pipe_host vs
// s4 16.5 and s8 34.
...
struct DpasS2ScNt2Name {};
struct DpasS2ScNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s2_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s2_sc: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS2ScNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
        return launch<DpasS2ScNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows, n,
                                              k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr, "dpas_s2_sc --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s2_sc: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s2_sc: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "wg=%dx%d_alongN padM=RC pack=4 a_scale=b_scale=%.4f out=f16 "
