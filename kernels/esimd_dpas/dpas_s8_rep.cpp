// K2 ESIMD s8, 8x2-N 64 dpas, in-kernel GEMM repeats for occupancy.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: heat-then-decode dropped clocks (61-65 us at ~730
// MHz). min_freq is root-only. Steal: one launch repeats the GEMM
// R times (zero acc each, store last) so GT stays busy while sysfs
// is sampled. us_per = event_us / R. Oracle on last store.
// CSV: phase,nt,unroll,reps,m,n,k,us,us_per,TOPS,max_abs,ok
...
struct DpasS8RepNt2Name {};
struct DpasS8RepNt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_rep: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
                   int32_t *cd, int rows, int cols, int dk, int reps) {
...
void run_shape(sycl::queue &q, int nt, int unroll, int reps, const char *phase,
               int m, int n, int k, int warmup, int iters, int *rc) {
    const int tn = nt * kExecN;
    const int inner_k = unroll * kK64;
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || reps < 1) {
        std::fprintf(stderr,
                     "dpas_s8_rep: shape m=%d n=%d k=%d nt=%d unroll=%d reps=%d\n",
                     m, n, k, nt, unroll, reps);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8RepNt4Name, 4, 8>(q, ad, bd, cd, rows, n, k,
                                                 reps);
        return launch<DpasS8RepNt2Name, 2, 16>(q, ad, bd, cd, rows, n, k, reps);
    };
    ...
    const double us_per = us / double(reps);
    const double ops = 2.0 * double(m) * double(n) * double(k);
    const double tops = (ops / 1.0e12) / (us_per * 1.0e-6);
    std::printf("%s,%d,%d,%d,%d,%d,%d,%.3f,%.3f,%.4f,%d,%d\n", phase, nt, unroll,
                reps, m, n, k, us, us_per, tops, max_abs, ok);
...
int main(int argc, char **argv) {
    ...
    int warmup = 1, iters = 3, reps = 4096;
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_rep --nt 2|4 [--m 1] [--reps 4096]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_rep: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_rep: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d wg=%dx%d_alongN padM=RC reps=%d warmup=%d iters=%d\n",
                backend, name.c_str(), driver.c_str(), kRc, kK64, kExecN, nt,
                nt * kExecN, unroll, 2 * nt * unroll, unroll * kK64, kWgX, kWgY,
                reps, warmup, iters);
    std::printf("phase,nt,unroll,reps,m,n,k,us,us_per,TOPS,max_abs,ok\n");
    int rc = 0;
    run_shape(q, nt, unroll, 2, "check", kRc, nt * kExecN, unroll * kK64, 1, 1,
              &rc);
    run_shape(q, nt, unroll, reps, "timed", timed_m, timed_n, timed_k, warmup,
              iters, &rc);
    return rc;
}
