// K2 ESIMD s8 RC=8, A-db, ngen-style wg 4x8, f16, M=64, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: sc8db 8x2-along-N A-db is 97-100 us vs W8A8 46.
// wg 4x8 (M on Y) was ~1.9x at M=256. Steal that geometry on the
// M=64 A-db tile: 4 along N, 8 along M. Same 64 dpas.8x8, k64,
// A ping-pong, no SLM. Rank pipe_host vs 97-100 and 46.
...
struct DpasS8Sc8db48Nt2Name {};
struct DpasS8Sc8db48Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8db48: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8db48: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8db48Nt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                      rows, n, k);
        return launch<DpasS8Sc8db48Nt2Name, 2, 16>(q, ad, bd, asd, bsd, cd,
                                                   rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8db48 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8db48: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8db48: --nt must be 2 or 4\n");
        return 2;
    }
