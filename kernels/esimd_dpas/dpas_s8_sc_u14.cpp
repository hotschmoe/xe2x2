// K8 ESIMD s8 RC=4, same 8x2-along-N scale-to-f16 tile as dpas_s8_sc,
// unroll=14 so inner_k=896 divides Lightning hidden 2688 (42*64).
// Stock NT=2 U=16 needs k % 1024 == 0; 2688 % 1024 = 640, refuse.
...
struct DpasS8ScU14Nt2Name {};
struct DpasS8ScU14Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc_u14: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc_u14: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8ScU14Nt4Name, 4, 6>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS8ScU14Nt2Name, 2, 14>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc_u14 --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc_u14: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc_u14: --nt must be 2 or 4\n");
        return 2;
    }
