// K7 leftover: ESIMD s8 RC=4 NT=1 split-K=2, scale-to-f16, o-proj.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: NT=1 is 55 vs W8A8 47. Packed SK=2 won M=64. wg 4x2
// 40 WGs along N lost (74). Steal SK=2 along K: 40 WGs, each K/2=3072
// (1024*3). Reduce is 4x5120 f32, tiny. Rank pipe_host vs 55 and 47.
...
struct DpasS8ScNt1skName {};
struct DpasS8Nt1skReduceName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc_nt1sk: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_sk(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    const size_t nwork = size_t(m_blocks * n_groups * kSK);
    const size_t g0 = round_up(int64_t(nwork), 16);
    const int k_half = dk / kSK;
    return q.parallel_for<Name>(
        sycl::nd_range<1>({g0}, {16}),
        [=](sycl::nd_item<1> it) SYCL_ESIMD_KERNEL {
            const size_t tid = size_t(it.get_global_id(0));
            if (tid >= nwork)
                return;
            const int64_t nplane = int64_t(m_blocks * n_groups);
            const int sk = int(tid / size_t(nplane));
            const int64_t rem = int64_t(tid % size_t(nplane));
            const int64_t mb = rem % m_blocks;
            const int64_t ng = rem / m_blocks;
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    const int k0 = k_lo + o * kInnerK + u * kK64;
...
#pragma unroll
            for (int t = 0; t < NT; ++t) {
                ...
                xesimd::lsc_store_2d<float, kExecN, kRc>(
                    cpart + plane, unsigned(cols * int(sizeof(float)) - 1),
                    unsigned(rows - 1),
                    unsigned(cols * int(sizeof(float)) - 1),
                    col0 + t * kExecN, row0, f);
            }
        });
}
...
sycl::event launch_reduce(sycl::queue &q, const float *cpart, const float *asd,
    ...
    return q.parallel_for<DpasS8Nt1skReduceName>(
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % (inner_k * kSK) != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc_nt1sk: shape m=%d n=%d k=%d nt=%d unroll=%d sk=%d\n",
                     m, n, k, nt, unroll, kSK);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        (void)launch_sk<DpasS8ScNt1skName, 1, 16>(q, ad, bd, cpart, rows, n, k);
...
int main(int argc, char **argv) {
    int nt = 1;
    int timed_m = 1, timed_n = 5120, timed_k = 6144;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc_nt1sk --nt 1 [--m 1] [--n 5120] [--k 6144]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc_nt1sk: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 1) {
        std::fprintf(stderr, "dpas_s8_sc_nt1sk: --nt must be 1\n");
        return 2;
    }
    const int unroll = 16;
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s8xs8->f16_scaled RC=%d NT=%d unroll=%d dpas=%d "
                ...
                g_card, g_spin);
    int rc = 0;
    run_shape(q, nt, unroll, "check", kRc, nt * kExecN, unroll * kK64 * kSK, 1,
              1, &rc, 0);
