// K7 ESIMD GDN mixer conv-L2-register T=256: one kernel, packed delta.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Q/K own full heads in registers; v keeps the channel-major mapping.
...
struct GdnMixerConvl2rConvName {};
struct GdnMixerConvl2rDeltaName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_mixer_convl2r: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_conv_l2(sycl::queue &q, const sycl::half *xd,
                           const sycl::half *wd, sycl::half *yd, int tlen) {
    constexpr int nwi_qk = kNk + kNk;
    constexpr int nwi_v = kCv / kVl;
    constexpr int nwi = nwi_qk + nwi_v;
    return q.parallel_for<GdnMixerConvl2rConvName>(
        sycl::nd_range<1>({size_t(nwi)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) SYCL_ESIMD_KERNEL {
            const int gid = int(it.get_global_id(0));
            if (gid < nwi_qk) {
                const int c0 = gid * kDk;
                esimd::simd<sycl::half, kVl> wv[kConv][kTiles];
#pragma unroll
                for (int k = 0; k < kConv; ++k) {
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile)
                        wv[k][tile].copy_from(wd + size_t(k) * size_t(kCtot) +
                                              size_t(c0 + tile * kVl));
                }
                esimd::simd<float, kVl> h0[kTiles];
                esimd::simd<float, kVl> h1[kTiles];
                esimd::simd<float, kVl> h2[kTiles];
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    h0[tile] = 0.f;
                    h1[tile] = 0.f;
                ...
                for (int t = 0; t < tlen; ++t) {
                    esimd::simd<float, kVl> xf[kTiles];
                    esimd::simd<float, kVl> acc[kTiles];
                    float tot = 0.f;
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        esimd::simd<sycl::half, kVl> xv;
                        xv.copy_from(xd + size_t(t) * size_t(kCtot) +
                                     size_t(c0 + tile * kVl));
                        xf[tile] = esimd::convert<float>(xv);
                        acc[tile] =
                            h0[tile] * esimd::convert<float>(wv[0][tile]) +
                            h1[tile] * esimd::convert<float>(wv[1][tile]) +
                            h2[tile] * esimd::convert<float>(wv[2][tile]) +
                            xf[tile] * esimd::convert<float>(wv[3][tile]);
                        tot += hsum16(acc[tile] * acc[tile]);
                    }
                    const float inv =
                        (tot > 0.f) ? 1.f / sycl::sqrt(tot) : 0.f;
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        const esimd::simd<float, kVl> yn = acc[tile] * inv;
                        esimd::simd<sycl::half, kVl> yh;
#pragma unroll
                        for (int e = 0; e < kVl; ++e)
                            yh[e] = sycl::half(float(yn[e]));
                        yh.copy_to(yd + size_t(t) * size_t(kCtot) +
                                   size_t(c0 + tile * kVl));
                        h0[tile] = h1[tile];
                        h1[tile] = h2[tile];
                        h2[tile] = xf[tile];
                ...
                const int c0 = kCq + kCk + (gid - nwi_qk) * kVl;
...
sycl::event launch_delta(sycl::queue &q, const sycl::half *sd,
                         ...
                         int tlen) {
    return q.parallel_for<GdnMixerConvl2rDeltaName>(
...
#pragma unroll
            ...
            for (int b = 0; b < nblk; ++b) {
                const int t0 = b * kBlk;
                const size_t ybase =
                    size_t(t0 + lid) * size_t(kCtot);
                const size_t qbase = ybase + size_t(kh) * size_t(kDk);
                const size_t kbase =
                    ybase + size_t(kCq) + size_t(kh) * size_t(kDk);
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    esimd::simd<sycl::half, kVl> qv, kv;
                    qv.copy_from(yd + qbase + size_t(tile * kVl));
                    kv.copy_from(yd + kbase + size_t(tile * kVl));
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_mixer_convl2r: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        (void)launch_conv_l2(q, xd, wd, yd, tlen);
        return launch_delta(q, sd, yd, ad, bd, s2d, od, tlen);
    };
    ...
    for (int i = 0; i < iters; ++i) {
        sycl::event ec = launch_conv_l2(q, xd, wd, yd, tlen);
        ec.wait_and_throw();
        sycl::event ed = launch_delta(q, sd, yd, ad, bd, s2d, od, tlen);
        ed.wait_and_throw();
    ...
    std::printf("phase,t,c,nv,event_us,wait_host_us,pipe_host_us,cosine,max_abs,"
                "cosine_o,max_abs_o,ok,median_us,min_us,max_us\n");
    std::printf("%s,%d,%d,%d,%.3f,%.3f,%.3f,%.6f,%.5g,%.6f,%.5g,%d,%.3f,%.3f,"
                "%.3f\n",
                phase, tlen, kCtot, kNv, us, wait_host, pipe_us, cosine, mx,
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_mixer_convl2r [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_mixer_convl2r: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_mixer_convl2r conv_l2r+delta dtype=f16 T=%d C=%d nv=%d "
                "blk=%d warmup=%d iters=%d card=%d spin=%d heat=none\n",
                backend, name.c_str(), driver.c_str(), timed_t, kCtot, kNv,
                kBlk, warmup, iters, g_card, g_spin);
    int rc = 0;
    run_shape(q, "timed", timed_t, warmup, iters, &rc);
    return rc;
}
