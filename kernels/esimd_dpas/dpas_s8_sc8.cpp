// K2 ESIMD s8 RC=8, 64 dpas.8x8, 8x2 along N, f16 scale epilogue.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: RC=4 scale-to-f16 wins M=1 (34 vs 44) and loses M=64
// (245 vs 46). ngen M=64 is dpas.8x8 RC=8 grf256. Steal RC=8 + 64
// unroll on the f16 contract. GRF256 requested; zebin may stay 128.
// Rank pipe_host vs W8A8 M=64 46 us. Fill s8 [-64,64], scales 0.02.
...
struct DpasS8Sc8Nt2Name {};
struct DpasS8Sc8Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8Nt4Name, 4, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                  n, k);
        return launch<DpasS8Sc8Nt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows, n,
                                               k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "wg=%dx%d_alongN grf256_request=1 a_scale=b_scale=%.4f out=f16 "
