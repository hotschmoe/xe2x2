// K7 ESIMD GDN mixer L2-once T=256: conv, L2 per kh, packed delta. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Device L2 once per (t,kh) not per delta WG. Rank vs mixer-slmht 471 vs L2-out 271.
...
struct GdnMixerL2onceConvName {};
struct GdnMixerL2onceL2Name {};
struct GdnMixerL2onceDeltaName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_mixer_l2once: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_conv(sycl::queue &q, const sycl::half *xd,
    ...
    return q.parallel_for<GdnMixerL2onceConvName>(
...
sycl::event launch_l2(sycl::queue &q, sycl::half *yd, int tlen) {
    const int nwi = tlen * kNk;
    return q.parallel_for<GdnMixerL2onceL2Name>(
        sycl::nd_range<1>({size_t(nwi)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) SYCL_ESIMD_KERNEL {
            const int gid = int(it.get_global_id(0));
            const int t = gid / kNk;
            const int kh = gid % kNk;
            const size_t ybase = size_t(t) * size_t(kCtot);
            const size_t qbase = ybase + size_t(kh) * size_t(kDk);
            const size_t kbase = ybase + size_t(kCq) + size_t(kh) * size_t(kDk);
            esimd::simd<sycl::half, kVl> qv[kTiles];
            esimd::simd<sycl::half, kVl> kv[kTiles];
            float qss = 0.f, kss = 0.f;
#pragma unroll
            for (int tile = 0; tile < kTiles; ++tile) {
                qv[tile].copy_from(yd + qbase + size_t(tile * kVl));
                kv[tile].copy_from(yd + kbase + size_t(tile * kVl));
                const esimd::simd<float, kVl> qf = esimd::convert<float>(qv[tile]);
                const esimd::simd<float, kVl> kf = esimd::convert<float>(kv[tile]);
                qss += hsum16(qf * qf);
                kss += hsum16(kf * kf);
            }
...
#pragma unroll
            for (int tile = 0; tile < kTiles; ++tile) {
                ...
                const esimd::simd<float, kVl> qf =
                    esimd::convert<float>(qv[tile]) * qinv;
                const esimd::simd<float, kVl> kf =
                    esimd::convert<float>(kv[tile]) * kinv;
#pragma unroll
                for (int e = 0; e < kVl; ++e) {
                    qn[e] = sycl::half(float(qf[e]));
                    kn[e] = sycl::half(float(kf[e]));
                }
                qn.copy_to(yd + qbase + size_t(tile * kVl));
                kn.copy_to(yd + kbase + size_t(tile * kVl));
            }
        });
}
...
sycl::event launch_delta(sycl::queue &q, const sycl::half *sd,
                         ...
                         int tlen) {
    return q.parallel_for<GdnMixerL2onceDeltaName>(
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_mixer_l2once: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        (void)launch_conv(q, xd, wd, yd, tlen);
        (void)launch_l2(q, yd, tlen);
    ...
    for (int i = 0; i < iters; ++i) {
        sycl::event ec = launch_conv(q, xd, wd, yd, tlen);
        ec.wait_and_throw();
        sycl::event el = launch_l2(q, yd, tlen);
        el.wait_and_throw();
        ...
        const uint64_t ns = span(ec) + span(el) + span(ed);
        ns_sum += ns;
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_mixer_l2once [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_mixer_l2once: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_mixer_l2once conv+l2+delta dtype=f16 T=%d C=%d nv=%d "
