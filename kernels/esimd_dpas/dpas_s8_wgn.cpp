// K2 ESIMD s8 RC=4, 64 dpas.8x4, wg 8x2 along N only (no idle M).
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: dpas_s8_wg mapped local {8,2}=(N,M) and idled half
// the WG at M=4 (warm 69 vs 1D u64 53-69). Steal: same 8x2 shape but
// both local dims index N (16 live N-groups per WG). M is group(1).
// Same per-thread NT tile as u64. No SLM, no extra ff.
...
struct DpasS8WgnNt2Name {};
struct DpasS8WgnNt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_wgn: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_wgn: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8WgnNt4Name, 4, 8>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8WgnNt2Name, 2, 16>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_wgn --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_wgn: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_wgn: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d wg=%dx%d_alongN Bload=Transformed warmup=%d iters=%d\n",
