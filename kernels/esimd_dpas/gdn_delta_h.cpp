// K7 ESIMD GDN fused-recurrent delta T=1 tree hsum. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Qwen3.8-27B: 48 v-heads, S 128x128 f16. T=1 decode.
// hsum via esimd::reduce not scalar. Prior: fused 7.1 at 2800.
// Rank pipe_host vs 7.1.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_h: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaHName>(
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--iters")
            ...
            std::fprintf(stderr, "gdn_delta_h [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_h: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_h_recurrent dtype=f16 nv=%d dv=%d dk=%d VL=%d "
