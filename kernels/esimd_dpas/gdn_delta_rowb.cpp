// K7 ESIMD GDN fused-recurrent delta T=256 row-block. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S like gdn_delta_t. 4 rows per WI, k reused in GRF.
// CONFIG prior: fused 1086 throttle=1. Rank pipe_host vs 1086.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_rowb: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaRowbName>(
...
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        ...
                        const esimd::simd<float, kVl> snew =
                            ah * (sf[r][tile] - bh * vold * kf) + bh * vi * kf;
                        sf[r][tile] = snew;
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < 1) {
        std::fprintf(stderr, "gdn_delta_rowb: T=%d\n", tlen);
        *rc = 2;
        return;
    }
    const size_t ns = size_t(kNv) * size_t(kDv) * size_t(kDk);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_rowb [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_rowb: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
