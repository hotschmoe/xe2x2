// K2 ESIMD s8 6-acc M-tile + wg 4x8 k128, f16, M=256, no SLM.
...
// ngen is 384x dpas.8x8 (16 acc = 4M x 4N x 24 k32; K=768 does
// not divide 5120). Steal the 384 count that does: 6x RC=8
// (48 rows/thread) NT=2 U=8 -> 384 dpas (6*4*2*8). Pad M 256->288.
// No A-db. B reused across M-tiles. Rank pipe_host vs 128 and 75.
...
struct DpasS8Sc8w48m6Nt2Name {};
struct DpasS8Sc8w48m6Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m6: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m6: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8w48m6Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                       rows, n, k);
        return launch<DpasS8Sc8w48m6Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                   n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m6 --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m6: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8w48m6: --nt must be 2 or 4\n");
        return 2;
    }
