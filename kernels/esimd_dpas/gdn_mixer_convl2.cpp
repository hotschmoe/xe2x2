// K7 ESIMD GDN mixer conv-L2 fuse T=256: conv+L2 one kernel, packed delta.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Drop the extra L2 launch vs L2-once 327. Rank vs seq conv+slmht ~298.
...
struct GdnMixerConvl2ConvName {};
struct GdnMixerConvl2DeltaName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_mixer_convl2: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_conv_l2(sycl::queue &q, const sycl::half *xd,
    ...
    return q.parallel_for<GdnMixerConvl2ConvName>(
        sycl::nd_range<1>({size_t(nwi)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) SYCL_ESIMD_KERNEL {
            esimd::slm_init<kWg * int(sizeof(float))>();
            const int gid = int(it.get_global_id(0));
            const int lid = int(it.get_local_id(0));
            const int c0 = gid * kVl;
            const bool do_l2 = c0 < (kCq + kCk);
...
#pragma unroll
            ...
            for (int t = 0; t < tlen; ++t) {
                ...
                esimd::simd<float, kVl> acc =
                    h0 * esimd::convert<float>(wv[0]) +
                    h1 * esimd::convert<float>(wv[1]) +
                    h2 * esimd::convert<float>(wv[2]) +
                    xf * esimd::convert<float>(wv[3]);
                if (do_l2) {
                    esimd::simd<float, 1> ss;
                    ss[0] = hsum16(acc * acc);
                    esimd::slm_block_store<float, 1>(lid * int(sizeof(float)),
                                                     ss);
                    it.barrier(sycl::access::fence_space::local_space);
                    float tot = 0.f;
                    const int base = (lid / kTiles) * kTiles;
#pragma unroll
                    for (int i = 0; i < kTiles; ++i) {
                        esimd::simd<float, 1> v = esimd::slm_block_load<float, 1>(
                            (base + i) * int(sizeof(float)));
                        tot += float(v[0]);
                    }
                    const float inv =
...
sycl::event launch_delta(sycl::queue &q, const sycl::half *sd,
                         ...
                         int tlen) {
    return q.parallel_for<GdnMixerConvl2DeltaName>(
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_mixer_convl2: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_mixer_convl2 [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_mixer_convl2: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_mixer_convl2 conv_l2+delta dtype=f16 T=%d C=%d nv=%d "
