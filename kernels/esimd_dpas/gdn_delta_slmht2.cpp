// K7 ESIMD GDN fused-recurrent delta T=256 tile-fused 2-row. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S. slmht leftover 260. Two i-rows per WI. Rank pipe_host vs 260.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_slmht2: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    constexpr int kI2 = 2;
    constexpr int kPairs = kDv / kI2;
    return q.parallel_for<GdnDeltaSlmht2Name>(
        sycl::nd_range<1>({size_t(kNv * kPairs)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) SYCL_ESIMD_KERNEL {
            esimd::slm_init<kSlmBytes>();
            const int gid = int(it.get_global_id(0));
            const int lid = int(it.get_local_id(0));
            const int h = gid / kPairs;
            const int i0 = (gid % kPairs) * kI2;
            const size_t srow0 =
                (size_t(h) * size_t(kDv) + size_t(i0)) * size_t(kDk);
            const size_t srow1 =
                (size_t(h) * size_t(kDv) + size_t(i0 + 1)) * size_t(kDk);
            esimd::simd<float, kVl> sf0[kTiles];
            esimd::simd<float, kVl> sf1[kTiles];
#pragma unroll
            for (int tile = 0; tile < kTiles; ++tile) {
                esimd::simd<sycl::half, kVl> sv0, sv1;
                sv0.copy_from(sd + srow0 + size_t(tile * kVl));
                sv1.copy_from(sd + srow1 + size_t(tile * kVl));
                sf0[tile] = esimd::convert<float>(sv0);
                sf1[tile] = esimd::convert<float>(sv1);
            }
            const int nblk = tlen / kBlk;
            for (int b = 0; b < nblk; ++b) {
                const int t0 = b * kBlk;
                const size_t kbase =
                    (size_t(t0 + lid) * size_t(kNv) + size_t(h)) * size_t(kDk);
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                ...
                for (int tt = 0; tt < kBlk; ++tt) {
                    ...
                    esimd::simd<float, kVl> vacc0 = 0.f;
                    esimd::simd<float, kVl> vacc1 = 0.f;
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                    ...
                    const float vold0 = hsum16(vacc0);
                    const float vold1 = hsum16(vacc1);
                    esimd::simd<sycl::half, 1> ahv, bhv;
                    esimd::simd<sycl::half, kI2> vv;
                    ahv.copy_from(ad + abase);
                    bhv.copy_from(bd + abase);
                    vv.copy_from(vd + vbase + size_t(i0));
                    const float ah = float(sycl::half(ahv[0]));
                    const float bh = float(sycl::half(bhv[0]));
                    const float vi0 = float(sycl::half(vv[0]));
                    const float vi1 = float(sycl::half(vv[1]));
                    esimd::simd<float, kVl> oaccv0 = 0.f;
                    esimd::simd<float, kVl> oaccv1 = 0.f;
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
                        const esimd::simd<float, kVl> kf =
                            esimd::convert<float>(kv[tile]);
                        const esimd::simd<float, kVl> qf =
                            esimd::convert<float>(qv[tile]);
                        const esimd::simd<float, kVl> snew0 =
                            ah * (sf0[tile] - bh * vold0 * kf) + bh * vi0 * kf;
                        const esimd::simd<float, kVl> snew1 =
                            ah * (sf1[tile] - bh * vold1 * kf) + bh * vi1 * kf;
                        sf0[tile] = snew0;
                        sf1[tile] = snew1;
                        oaccv0 += snew0 * qf;
                        oaccv1 += snew1 * qf;
                    }
                    esimd::simd<sycl::half, kI2> ov;
                    ov[0] = sycl::half(hsum16(oaccv0));
                    ov[1] = sycl::half(hsum16(oaccv1));
                    ov.copy_to(od + vbase + size_t(i0));
                }
                it.barrier(sycl::access::fence_space::local_space);
            }
#pragma unroll
            for (int tile = 0; tile < kTiles; ++tile) {
                esimd::simd<sycl::half, kVl> sh0, sh1;
#pragma unroll
                for (int e = 0; e < kVl; ++e) {
                    sh0[e] = sycl::half(float(sf0[tile][e]));
                    sh1[e] = sycl::half(float(sf1[tile][e]));
                }
                sh0.copy_to(s2d + srow0 + size_t(tile * kVl));
                sh1.copy_to(s2d + srow1 + size_t(tile * kVl));
            }
        });
}
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_slmht2: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_slmht2 [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_slmht2: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_slmht2 dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d blk=%d "
                "warmup=%d iters=%d card=%d spin=%d heat=none\n",
                backend, name.c_str(), driver.c_str(), timed_t, kNv, kDv, kDk,
                kVl, kBlk, warmup, iters, g_card, g_spin);
    int rc = 0;
