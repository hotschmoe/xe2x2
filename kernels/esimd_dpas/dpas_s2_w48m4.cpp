// K2 ESIMD s2 4-acc M-tile + wg 4x8 k128, f16, M=256, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s4 4-acc is 48.6 us at M=256 vs s2 4x8 A-db 55.5 vs
// W8A8 75. Steal native s2 onto this 4-acc tile: packed 4/byte along
// K, two s2 dpas per k128. Same 4 M-tiles, wg 4x8, no A-db, no SLM.
// IGC s2 [-2,1]. Never E2M1 bitcast. Rank pipe_host vs 48.6 and 55.5.
...
struct DpasS2W48m4Nt2Name {};
struct DpasS2W48m4Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s2_w48m4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s2_w48m4: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS2W48m4Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS2W48m4Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s2_w48m4 --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s2_w48m4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s2_w48m4: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "dpas=%d wg=%dx%d_NxM no_slm=1 pack=4 "
