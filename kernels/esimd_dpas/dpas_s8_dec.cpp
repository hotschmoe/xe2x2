// K2 ESIMD s8 RC=4, 64 dpas.8x4, 8x2 along N, M=1 pad to RC=4.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: ngen M=1 SLM is d32 store/load/fence (14 ops), not
// A-tile pack. wgn floor is 47-50 us at pad M=4 vs W8A8 M=1 42-46.
// Steal: same wgn tile, pad M=1 with zero rows to RC=4 (ngen does
// RC=4 at M=1). Rank us vs 45. Oracle is the real M=1 rows.
...
struct DpasS8DecNt2Name {};
struct DpasS8DecNt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_dec: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_dec: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8DecNt4Name, 4, 8>(q, ad, bd, cd, rows, n, k);
        return launch<DpasS8DecNt2Name, 2, 16>(q, ad, bd, cd, rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_dec --nt 2|4 [--m 1] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_dec: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_dec: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d wg=%dx%d_alongN padM=RC Bload=Transformed warmup=%d iters=%d\n",
