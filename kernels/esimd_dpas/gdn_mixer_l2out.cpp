// K7 ESIMD GDN mixer L2-out T=256: packed slmht delta, host L2. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Host conv+L2 q/k. Device packed delta skips L2. Rank vs slmht 260 vs mixer 471.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_mixer_l2out: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_delta(sycl::queue &q, const sycl::half *sd,
                         ...
                         int tlen) {
    return q.parallel_for<GdnMixerL2outDeltaName>(
...
void host_l2_packed_qk(sycl::half *y, int tlen) {
    ...
    for (int t = 0; t < tlen; ++t) {
        ...
        for (int kh = 0; kh < kNk; ++kh) {
            ...
            for (int j = 0; j < kDk; ++j) {
                y[ybase + size_t(kh) * size_t(kDk) + size_t(j)] =
                    sycl::half(qn[size_t(j)]);
                y[ybase + size_t(kCq) + size_t(kh) * size_t(kDk) + size_t(j)] =
                    sycl::half(kn[size_t(j)]);
            }
        }
    }
}
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_mixer_l2out: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
    ...
    host_delta(hs.data(), hyref.data(), ha.data(), hb.data(), hrefs.data(),
               hrefo.data(), tlen);
    host_l2_packed_qk(hyref.data(), tlen);
    ...
    q.memcpy(yd, hyref.data(), nx * sizeof(sycl::half)).wait();
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        ed.wait_and_throw();
        const uint64_t t0 =
            ed.get_profiling_info<sycl::info::event_profiling::command_start>();
        const uint64_t t1 =
            ed.get_profiling_info<sycl::info::event_profiling::command_end>();
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_mixer_l2out [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_mixer_l2out: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_mixer_l2out packed-delta host-L2 dtype=f16 T=%d C=%d "
                "nv=%d blk=%d warmup=%d iters=%d card=%d spin=%d heat=none\n",
