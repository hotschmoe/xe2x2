// K7 leftover: ESIMD s8 4-acc wg 4x8 k128 split-K=2, f16, packed qkv.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: packed qkv M=256 4-acc 4x8 is 274 vs W8A8 164. wg 8x4
// 279. persist B-pipe 344. Steal SK=2: each WI does K/2, write f32
// partials, reduce+scale to f16. 2x K occupancy. unroll=5 innerK=640
// so 2560 divides. Rank pipe_host of gemm+reduce vs 274 and 164.
...
struct DpasS8Sc8w48m4skNt2Name {};
struct DpasS8SkReduceName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4sk: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_reduce(sycl::queue &q, const float *cpart, const float *asd,
    ...
    return q.parallel_for<DpasS8SkReduceName>(
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % (inner_k * kSK) != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m4sk: shape m=%d n=%d k=%d nt=%d unroll=%d sk=%d\n",
                     m, n, k, nt, unroll, kSK);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        (void)launch_sk<DpasS8Sc8w48m4skNt2Name, 2, 5>(q, ad, bd, cpart, rows,
                                                       n, k);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m4sk --nt 2 [--m 256] [--n 10240]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m4sk: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4sk: --nt must be 2\n");
        return 2;
    }
    const int unroll = 5;
