// K2 ESIMD s2xs8 RC=4, 8x2 along N, scale-to-f16, M=1 pad, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Literature mix (arXiv 2508.06753): B=s2 packed 4/byte, A=s8,
// dpas K=32 (OPC=4, same systolic rate as s8). CONFIG prior: ~34 us
// like s8, not 11.5 like s2xs2. IGC s2 [-2,1]. Never E2M1 bitcast.
// Rank pipe_host vs s2 11.5, s4 16.5, s8 34.
...
struct DpasS2xS8ScNt2Name {};
struct DpasS2xS8ScNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s2xs8_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
                        ...
                        acc[t] = xmx::dpas<8, kRc, int32_t, int32_t, uint8_t,
                                           int8_t, xmx::dpas_argument_type::s2,
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPackB != 0) {
        std::fprintf(stderr,
                     "dpas_s2xs8_sc: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS2xS8ScNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS2xS8ScNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s2xs8_sc --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s2xs8_sc: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s2xs8_sc: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "wg=%dx%d_alongN padM=RC packB=4 a_scale=b_scale=%.4f out=f16 "
