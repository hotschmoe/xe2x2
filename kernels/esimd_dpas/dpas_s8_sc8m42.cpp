// K2 ESIMD s8 4-acc + wg 4x2 (M on Y), k64 A-db, f16, M=64, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: sc8m4 4-acc wg 8x2-along-N is 120 us vs wg 4x8
// A-db 75 vs W8A8 46. M-on-Y won at M=256 4-acc (128 vs 228) and
// at M=64 8-row (75 vs 97). Steal that geometry on 4-acc: wg 4
// along N x 2 along M. 32 rows/thread * 2 = 64, no idle. Same
// 64 dpas, k64 A-db, no SLM. Rank pipe_host vs 120 and 75.
...
struct DpasS8Sc8m42Nt2Name {};
struct DpasS8Sc8m42Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8m42: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8m42: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8m42Nt4Name, 4, 2>(q, ad, bd, asd, bsd, cd,
                                                     rows, n, k);
        return launch<DpasS8Sc8m42Nt2Name, 2, 4>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8m42 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8m42: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8m42: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "unroll=%d dpas=%d wg=%dx%d_NxM A_double_buffer=1 no_slm=1 "
