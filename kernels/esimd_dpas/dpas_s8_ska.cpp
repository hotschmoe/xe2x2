// K2 ESIMD s8 RC=4, 64 dpas.8x4 + software-pipelined A loads.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: ngen M=1 catalog has ska. ff overlap (dpas_s8_ov) still
// lost decode to no-pf u64 (100-264 vs 53-69 vs W8A8 45). Steal a real
// double-buffer: prologue load A[k=0], issue A[k+64] before dpas of
// current A so the next A is in GRF, not a null-dest prefetch.
// Same NT=2 U=16 / NT=4 U=8 as u64. No extra ff. No SLM.
...
struct DpasS8SkaNt2Name {};
struct DpasS8SkaNt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_ska: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_ska: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8SkaNt4Name, 4, 8>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8SkaNt2Name, 2, 16>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_ska --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_ska: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_ska: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d pipe=A-double-buffer Bload=Transformed "
                "warmup=%d iters=%d\n",
