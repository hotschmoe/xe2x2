// K2 ESIMD s8 RC=8, A+B double-buffer, ca.ca loads, f16, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: sc8db A-db is 97-100 us vs W8A8 46. ngen M=64 loads
// A/B with ca.ca and pipelines B in GRF (xaf), not SLM. Steal: keep
// the 8-row A ping-pong tile, add B ping-pong + cached/cached on
// lsc_load_2d. No prefetch, no SLM. Rank pipe_host vs 97-100 and 46.
...
struct DpasS8Sc8bpNt2Name {};
struct DpasS8Sc8bpNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8bp: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
            for (int t = 0; t < NT; ++t)
                acc[t] = 0;
            constexpr auto kL1 = xesimd::cache_hint::cached;
            constexpr auto kL2 = xesimd::cache_hint::cached;
            esimd::simd<int8_t, kRc * kKc> a0 =
                xesimd::lsc_load_2d<int8_t, kKc, kRc, 1, false, false, kL1,
                                    kL2>(ad, unsigned(dk - 1),
                                         unsigned(rows - 1), unsigned(dk - 1),
                                         0, row0);
            esimd::simd<int8_t, kRc * kKc> a1 =
                xesimd::lsc_load_2d<int8_t, kKc, kRc, 1, false, false, kL1,
                                    kL2>(ad, unsigned(dk - 1),
                                         unsigned(rows - 1), unsigned(dk - 1),
                                         kKc, row0);
            esimd::simd<int8_t, kKc * kExecN> b0[NT];
            esimd::simd<int8_t, kKc * kExecN> b1[NT];
#pragma unroll
            for (int t = 0; t < NT; ++t) {
                b0[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1, false, true,
                                            kL1, kL2>(
                    bd, unsigned(cols - 1), unsigned(dk - 1), unsigned(cols - 1),
                    col0 + t * kExecN, 0);
                b1[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1, false, true,
                                            kL1, kL2>(
                    bd, unsigned(cols - 1), unsigned(dk - 1), unsigned(cols - 1),
                    col0 + t * kExecN, kKc);
            }
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    const int knext = more_u ? (k0 + kK64) : ((o + 1) * kInnerK);
                    const bool load_next = more_u || (o + 1 < outer);
                    esimd::simd<int8_t, kRc * kKc> a0n = a0;
                    esimd::simd<int8_t, kRc * kKc> a1n = a1;
                    esimd::simd<int8_t, kKc * kExecN> b0n[NT];
                    esimd::simd<int8_t, kKc * kExecN> b1n[NT];
                    if (load_next) {
                        a0n = xesimd::lsc_load_2d<int8_t, kKc, kRc, 1, false,
                                                  false, kL1, kL2>(
                            ad, unsigned(dk - 1), unsigned(rows - 1),
                            unsigned(dk - 1), knext, row0);
                        a1n = xesimd::lsc_load_2d<int8_t, kKc, kRc, 1, false,
                                                  false, kL1, kL2>(
                            ad, unsigned(dk - 1), unsigned(rows - 1),
                            unsigned(dk - 1), knext + kKc, row0);
#pragma unroll
                        for (int t = 0; t < NT; ++t) {
                            b0n[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1,
                                                         false, true, kL1, kL2>(
                                bd, unsigned(cols - 1), unsigned(dk - 1),
                                unsigned(cols - 1), col0 + t * kExecN, knext);
                            b1n[t] = xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1,
                                                         false, true, kL1, kL2>(
                                bd, unsigned(cols - 1), unsigned(dk - 1),
...
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
                        acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t,
                                                  int8_t, int8_t>(acc[t], b0[t],
                                                                  a0);
                        acc[t] = esimd::xmx::dpas<8, kRc, int32_t, int32_t,
                                                  int8_t, int8_t>(acc[t], b1[t],
                                                                  a1);
                    }
                    if (load_next) {
                        a0 = a0n;
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8bp: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8bpNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS8Sc8bpNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8bp --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8bp: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8bp: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "wg=%dx%d_alongN A_double_buffer=1 B_double_buffer=1 "
                "lsc_ca_ca=1 no_slm=1 "
