// K2 ESIMD s8 RC=4, 64 dpas.8x4, 8x2 along N, ngen d32 flag broadcast.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: ngen M=1 SLM is 14x store/load/fence.slm.d32 plus
// send.gtwy barrier, not A-pack. Pattern: predicated store.d32,
// fence.slm.group, gtwy barrier, predicated load.d32. Steal that
// prologue once per launch on the wgn 8x2-N 64-dpas tile. Token is
// live in k0 so IGC cannot DCE it (token is 0). Rank us vs wgn 47-50
// and W8A8 42-46. Pad M=1 to RC=4.
...
struct DpasS8D32Nt2Name {};
struct DpasS8D32Nt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_d32: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    return q.parallel_for<Name>(
        ...
        [=](sycl::nd_item<2> it) SYCL_ESIMD_KERNEL {
            ...
            if (lid == 0) {
                esimd::simd<int32_t, 1> tok = 0;
                esimd::slm_block_store<int32_t, 1>(0, tok);
            }
            esimd::barrier();
            const esimd::simd<int32_t, 1> got =
                esimd::slm_block_load<int32_t, 1>(0);
            const int token = int(got[0]);
            if (!live)
                return;
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    const int k0 = o * kInnerK + u * kK64 + token;
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_d32: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8D32Nt4Name, 4, 8>(q, ad, bd, cd, rows, n, k);
        return launch<DpasS8D32Nt2Name, 2, 16>(q, ad, bd, cd, rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_d32 --nt 2|4 [--m 1] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_d32: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_d32: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d wg=%dx%d_alongN padM=RC slm=d32_flag Bload=Transformed "
                "warmup=%d iters=%d\n",
