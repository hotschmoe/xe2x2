// K2 ESIMD s8 4-acc + k32 A-db + wg 4x8 k128, f16, M=256, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: 4-acc wg 4x8 k128 is 128 us at M=256 vs W8A8 75.
// A-db won at M=64 (75 vs 97). 384-count 6-acc lost (210).
// Steal ska-style k32 A ping-pong on the 4-acc tile: prologue
// A[k=0], issue A[k+32] before dpas of current. Keep 256 dpas.
// Not full k128 A-db (that is 16 A tiles x2, GRF blowup).
// Rank pipe_host vs 128 and 75.
...
struct DpasS8Sc8w48m4dbNt2Name {};
struct DpasS8Sc8w48m4dbNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4db: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
                    for (int i = 0; i < kKPer; ++i) {
                        ...
                        const bool more = (u + 1 < kUnroll) || (i + 1 < kKPer) ||
                                          (o + 1 < outer);
                        esimd::simd<int8_t, kRc * kKc> an[kMTiles];
                        if (more) {
#pragma unroll
                            for (int r = 0; r < kMTiles; ++r)
                                an[r] = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                    ad, unsigned(dk - 1), unsigned(rows - 1),
                                    unsigned(dk - 1), kk + kKc,
                                    row0 + r * kRc);
                        }
#pragma unroll
                        for (int t = 0; t < NT; ++t) {
                            const esimd::simd<int8_t, kKc * kExecN> b =
                                xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1,
                                    ...
                                    unsigned(cols - 1), col0 + t * kExecN, kk);
#pragma unroll
                            for (int r = 0; r < kMTiles; ++r)
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m4db: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8w48m4dbNt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                        rows, n, k);
        return launch<DpasS8Sc8w48m4dbNt2Name, 2, 8>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m4db --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m4db: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4db: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "dpas=%d wg=%dx%d_NxM a_db=1 no_slm=1 "
