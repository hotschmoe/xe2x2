// K2 ESIMD DPAS s8 RC=4. Backend: sycl+l0. AOT intel_gpu_bmg_g31.
// Standalone icpx -fsycl (intel/llvm#21741).
//
// CONFIG prior: oneDNN W8A8 M=1 uses dpas.8x4 (RC=4), not RC=8.
// Pad decode M=1 to M=4 instead of M=8. Same Transformed B load.
...
struct DpasS8Rc4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_rc4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    return q.parallel_for<DpasS8Rc4Name>(
...
void run_shape(sycl::queue &q, const char *phase, int m, int n, int k,
    ...
    if (m % kRc != 0 || n % kExecN != 0 || k % kKc != 0) {
        std::fprintf(stderr, "dpas_s8_rc4: shape m=%d n=%d k=%d\n", m, n, k);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--m")
            ...
            std::fprintf(stderr, "dpas_s8_rc4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_rc4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
