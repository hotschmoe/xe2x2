// K7 ESIMD GDN fused-recurrent delta update. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Qwen3.8-27B: 48 v-heads, S 128x128 f16. T=1 decode.
// S' = a*(S - b*vold k^T) + b*v k^T; vold = S k; o = S' q.
// CONFIG prior: eager bmm path ~308 us. Rank pipe_host vs 308.
// One WI per (head, row). f16 in/out, float acc.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaName>(
...
#pragma unroll
            for (int t = 0; t < kTiles; ++t) {
                ...
                kv[t].copy_from(kd + kbase + size_t(t * kVl));
                qv[t].copy_from(qd + kbase + size_t(t * kVl));
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--iters")
            ...
            std::fprintf(stderr, "gdn_delta [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_recurrent dtype=f16 nv=%d dv=%d dk=%d VL=%d "
