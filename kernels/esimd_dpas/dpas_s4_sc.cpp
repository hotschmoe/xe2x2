// K2 ESIMD s4 RC=4, 8x2 along N, scale-to-f16, M=1 pad, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s8 RC=4 8x2-N scale-to-f16 is 34 us at M=1 vs W8A8 44.
// s4 4x8 A-db was 33.6 us at M=64. Steal native s4 onto the decode
// tile: packed 2/byte along K, one s4 dpas per k64 (s8 used two k32).
// Same wg 8x2 along N, pad M to RC=4. Never E2M1 bitcast. s4 [-8,7].
// Rank pipe_host vs 34 and 44.
...
struct DpasS4ScNt2Name {};
struct DpasS4ScNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s4_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s4_sc: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS4ScNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
        return launch<DpasS4ScNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows, n,
                                              k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr, "dpas_s4_sc --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s4_sc: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s4_sc: --nt must be 2 or 4\n");
        return 2;
    }
