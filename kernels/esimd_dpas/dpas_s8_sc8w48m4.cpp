...
//
// CONFIG prior: wg 4x8 k128 8-row is 228 us at M=256 vs W8A8 75.
// ngen is 384x dpas.8x8. Steal 4x RC=8 (32 rows/thread) on that
// geometry so 256 dpas (4*4*2*8). No A-db (GRF). B reused across
// M-tiles. Rank pipe_host vs 228 and 75.
...
struct DpasS8Sc8w48m4Nt2Name {};
struct DpasS8Sc8w48m4Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m4: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8w48m4Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                       rows, n, k);
        return launch<DpasS8Sc8w48m4Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                   n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m4 --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4: --nt must be 2 or 4\n");
        return 2;
    }
