// K2 ESIMD s8xs4 RC=4, 8x2 along N, scale-to-f16, M=1 pad, no SLM.
...
struct DpasS8xS4ScNt2Name {};
struct DpasS8xS4ScNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8xs4_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPackB != 0) {
        std::fprintf(stderr,
                     "dpas_s8xs4_sc: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8xS4ScNt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS8xS4ScNt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8xs4_sc --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8xs4_sc: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8xs4_sc: --nt must be 2 or 4\n");
        return 2;
    }
