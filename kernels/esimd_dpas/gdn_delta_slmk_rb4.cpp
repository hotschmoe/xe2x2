// K7 ESIMD GDN fused-recurrent delta T=256 SLM-K + rb=4. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Block-16 k/q in SLM plus 4 rows per WI. SLM-K 847, rb=4 1034.
// Rank pipe_host vs 847.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_slmk_rb4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaSlmkRb4Name>(
        sycl::nd_range<1>({size_t(kNv * kRowWIs)}, {size_t(kWg)}),
        [=](sycl::nd_item<1> it) SYCL_ESIMD_KERNEL {
            esimd::slm_init<kSlmBytes>();
            const int gid = int(it.get_global_id(0));
            const int lid = int(it.get_local_id(0));
            const int h = gid / kRowWIs;
            const int i0 = (gid % kRowWIs) * kRb;
            esimd::simd<float, kVl> sf[kRb][kTiles];
#pragma unroll
            for (int r = 0; r < kRb; ++r) {
                const size_t srow =
                    (size_t(h) * size_t(kDv) + size_t(i0 + r)) * size_t(kDk);
#pragma unroll
                for (int tile = 0; tile < kTiles; ++tile) {
                    esimd::simd<sycl::half, kVl> sv;
                    sv.copy_from(sd + srow + size_t(tile * kVl));
                    sf[r][tile] = esimd::convert<float>(sv);
                }
            }
...
#pragma unroll
                        for (int tile = 0; tile < kTiles; ++tile)
                            vold += hsum16(sf[r][tile] *
                                           esimd::convert<float>(kv[tile]));
                        esimd::simd<sycl::half, 1> viv;
                        viv.copy_from(vd + vbase + size_t(i0 + r));
...
#pragma unroll
                        ...
                        ov[0] = sycl::half(oacc);
                        ov.copy_to(od + vbase + size_t(i0 + r));
                    }
                }
                it.barrier(sycl::access::fence_space::local_space);
            }
...
#pragma unroll
                    for (int e = 0; e < kVl; ++e)
                        sh[e] = sycl::half(float(sf[r][tile][e]));
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_slmk_rb4: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_slmk_rb4 [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_slmk_rb4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_slmk_rb4 dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d "
                "blk=%d rb=%d warmup=%d iters=%d card=%d spin=%d heat=none\n",
                backend, name.c_str(), driver.c_str(), timed_t, kNv, kDv, kDk,
                kVl, kBlk, kRb, warmup, iters, g_card, g_spin);
    int rc = 0;
