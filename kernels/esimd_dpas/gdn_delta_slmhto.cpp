// K7 ESIMD GDN fused-recurrent delta T=256 tile-fused packed o. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S. slmht leftover 260. Scalar o store becomes 16-wide along Dv.
...
constexpr int kOffO = kOffQ + kBlk * kDk * int(sizeof(sycl::half));
constexpr int kSlmBytes = kOffO + kBlk * kWg * int(sizeof(sycl::half));

struct GdnDeltaSlmhtoName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_slmhto: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaSlmhtoName>(
...
#pragma unroll
                    ...
                    esimd::slm_block_store<sycl::half, 1>(
                        kOffO + (tt * kWg + lid) * int(sizeof(sycl::half)), ov);
                }
                it.barrier(sycl::access::fence_space::local_space);
                {
                    const int i0 = i - lid;
                    esimd::simd<sycl::half, kWg> ov;
                    ov = esimd::slm_block_load<sycl::half, kWg>(
                        kOffO + lid * kWg * int(sizeof(sycl::half)));
                    const size_t obase =
                        (size_t(t0 + lid) * size_t(kNv) + size_t(h)) *
                        size_t(kDv);
                    ov.copy_to(od + obase + size_t(i0));
                }
            }
#pragma unroll
            for (int tile = 0; tile < kTiles; ++tile) {
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_slmhto: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_slmhto [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_slmhto: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_slmhto dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d blk=%d "
