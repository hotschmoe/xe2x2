// K2 ESIMD s4 RC=8, A-db, ngen-style wg 4x8, f16, M=64, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s8 wg 4x8 A-db is the M=64 hand floor 75 us vs W8A8 46.
// s4 DPAS was 1.49x s8 at 1024^3 / ~583 MHz, not 2x. Steal that 4x8
// A-db tile onto native s4 (packed 2/byte along K, Transformed B).
// One s4 dpas covers K=64 (s8 used two k32). Same wg 4x8, NT=2
// unroll=16, k64 A ping-pong, no SLM. Never E2M1 bitcast. s4 [-8,7].
// Rank pipe_host vs 75 and 46.
...
constexpr int kRc = 8;
constexpr int kKc = 64; // s4 dpas K
...
struct DpasS4Db48Nt2Name {};
struct DpasS4Db48Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s4_db48: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
                ...
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    if (more_u) {
                        a0n = xesimd::lsc_load_2d<uint8_t, kAPacked, kRc>(
                            ad, unsigned(a_pitch - 1), unsigned(rows - 1),
                            unsigned(a_pitch - 1), (k0 + kK64) / kPack, row0);
                    } else if (o + 1 < outer) {
                        const int kn = (o + 1) * kInnerK;
                        a0n = xesimd::lsc_load_2d<uint8_t, kAPacked, kRc>(
                            ad, unsigned(a_pitch - 1), unsigned(rows - 1),
                            unsigned(a_pitch - 1), kn / kPack, row0);
                    }
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s4_db48: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS4Db48Nt4Name, 4, 8>(q, ad, bd, asd, bsd, cd,
                                                   rows, n, k);
        return launch<DpasS4Db48Nt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s4_db48 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s4_db48: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s4_db48: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s4xs4->f16_scaled RC=%d NT=%d unroll=%d dpas=%d "
