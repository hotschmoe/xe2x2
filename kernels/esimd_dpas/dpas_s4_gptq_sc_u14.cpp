// K8 ESIMD s4 RC=4 GPTQ-style group-scale f16 decode.
...
//
// Same wg 8x2 as dpas_s4_sc. B is s4 with g128 f16 scales.
// Without --b-bin, deterministic synthetic s4 weights and scales are used.
// Partial s32 per group, then * a_scale * b_scale[g,n]. Never E2M1.
// Rank pipe_host vs s4 16.5 and W8A8 44.
...
struct DpasS4GptqScU14Nt2Name {};
struct DpasS4GptqScU14Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s4_gptq_sc_u14: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kGs != 0 ||
        ...
        std::fprintf(stderr,
                     "dpas_s4_gptq_sc_u14: shape m=%d n=%d k=%d nt=%d unroll=%d\n",
                     m, n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS4GptqScU14Nt4Name, 4, 6>(q, ad, bd, asd, sd, cd,
                                                     rows, n, k);
        return launch<DpasS4GptqScU14Nt2Name, 2, 14>(q, ad, bd, asd, sd, cd, rows,
                                                  n, k);
    };
...
int load_dump(const char *path, std::vector<int8_t> *hb,
    ...
    if (!f) {
        std::fprintf(stderr, "dpas_s4_gptq_sc_u14: open %s\n", path);
        return 2;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s4_gptq_sc_u14 [--b-bin file] --nt 2|4 "
                         "[--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s4_gptq_sc_u14: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    std::vector<int8_t> hb_store;
    std::vector<sycl::half> hs_store;
    const char *weight_source = "dump";
    if (bin) {
        if (load_dump(bin, &hb_store, &hs_store, &g_dump_k, &g_dump_n,
                      &g_dump_gs) != 0)
            return 2;
        if (g_dump_gs != kGs) {
            std::fprintf(stderr, "dpas_s4_gptq_sc_u14: gs %d != %d\n",
                         g_dump_gs, kGs);
            return 2;
        }
    } else {
        ...
        g_dump_n = timed_n;
        g_dump_gs = kGs;
        hb_store.resize(size_t(g_dump_k) * size_t(g_dump_n));
        fill_s4(hb_store.data(), hb_store.size(), 9);
        hs_store.assign(size_t(g_dump_k / kGs) * size_t(g_dump_n),
                        sycl::half(kScale));
    }
    g_hb = hb_store.data();
    g_hs = hs_store.data();
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s4_gptq_sc_u14: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s4xs4_gptq->f16 weights=%s RC=%d NT=%d unroll=%d gs=%d "
                "wg=%dx%d_alongN padM=RC pack=2 a_scale=%.4f dump_k=%d dump_n=%d "
                "out=f16 warmup=%d iters=%d card=%d spin=%d heat=none\n",
                backend, name.c_str(), driver.c_str(), weight_source, kRc, nt,
                unroll, kGs,
