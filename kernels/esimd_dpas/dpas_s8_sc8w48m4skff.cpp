// K7 leftover: ESIMD s8 4-acc wg 4x8 k128 split-K=2 + lsc_prefetch, packed qkv.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: packed M=256 4-acc 274, prefetch 267, SK=2 295 vs W8A8 164.
// Steal SK=2 occupancy plus lsc_prefetch_2d of next k128 A/B inside each
// K-half. unroll=5 innerK=640 so 2560 divides. Rank pipe_host vs 267 and 164.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4skff: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_sk(sycl::queue &q, const int8_t *ad, const int8_t *bd,
                      float *cpart, int rows, int cols, int dk) {
    ...
    return q.parallel_for<Name>(
        ...
        [=](sycl::nd_item<2> it) SYCL_ESIMD_KERNEL {
            ...
            const int64_t mb_sk = int64_t(it.get_group(1)) * kWgY +
                                  int64_t(it.get_local_id(1));
            if (ng >= n_groups || mb_sk >= m_sk)
                return;
            const int64_t mb = mb_sk % m_blocks;
            const int sk = int(mb_sk / m_blocks);
            const int row0 = int(mb * kRowsTh);
            const int col0 = int(ng * kTN);
            const int k_lo = sk * k_half;
...
#pragma unroll
                for (int t = 0; t < NT; ++t)
                    acc[r][t] = 0;
            const int outer = k_half / kInnerK;
            const int k_hi = k_lo + k_half;
            for (int o = 0; o < outer; ++o) {
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    const int k0 = k_lo + o * kInnerK + u * kK128;
                    const int kn = k0 + kK128;
                    if (kn < k_hi) {
...
#pragma unroll
                        for (int t = 0; t < NT; ++t)
                            xesimd::lsc_prefetch_2d<
                                ...
                                unsigned(cols - 1), col0 + t * kExecN, kn);
                    }
#pragma unroll
                    for (int i = 0; i < kKPer; ++i) {
                        esimd::simd<int8_t, kRc * kKc> a[kMTiles];
#pragma unroll
                        for (int r = 0; r < kMTiles; ++r)
                            a[r] = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                ad, unsigned(dk - 1), unsigned(rows - 1),
                                unsigned(dk - 1), k0 + i * kKc,
                                row0 + r * kRc);
#pragma unroll
                        for (int t = 0; t < NT; ++t) {
                            const esimd::simd<int8_t, kKc * kExecN> b =
                                xesimd::lsc_load_2d<int8_t, kExecN, kKc, 1,
                                                    false, true>(
                                    bd, unsigned(cols - 1), unsigned(dk - 1),
...
#pragma unroll
            ...
            const size_t plane = size_t(sk) * size_t(rows) * size_t(cols);
#pragma unroll
            for (int r = 0; r < kMTiles; ++r) {
#pragma unroll
                for (int t = 0; t < NT; ++t) {
                    esimd::simd<float, kRc * kExecN> f(acc[r][t]);
                    xesimd::lsc_store_2d<float, kExecN, kRc>(
                        cpart + plane,
                        unsigned(cols * int(sizeof(float)) - 1),
                        unsigned(rows - 1),
                        unsigned(cols * int(sizeof(float)) - 1),
                        col0 + t * kExecN, row0 + r * kRc, f);
                }
            }
        });
}

sycl::event launch_reduce(sycl::queue &q, const float *cpart, const float *asd,
                          const float *bsd, sycl::half *cd, int rows, int cols) {
    const size_t n = size_t(rows) * size_t(cols);
    const size_t g = round_up(int64_t(n), 256);
    return q.parallel_for<DpasS8SkffReduceName>(
        sycl::nd_range<1>({g}, {256}), [=](sycl::nd_item<1> it) {
            const size_t i = size_t(it.get_global_id(0));
            if (i >= n)
                return;
            const int row = int(i / size_t(cols));
            const int col = int(i % size_t(cols));
            float s = 0.f;
            for (int sk = 0; sk < kSK; ++sk)
                s += cpart[size_t(sk) * n + i];
            cd[i] = sycl::half(s * asd[row] * bsd[col]);
        });
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % (inner_k * kSK) != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8w48m4skff: shape m=%d n=%d k=%d nt=%d unroll=%d sk=%d\n",
                     m, n, k, nt, unroll, kSK);
        *rc = 2;
        return;
    }
    ...
    float *cpart = sycl::malloc_device<float>(size_t(kSK) * nc_pad, q);
    ...
    auto go = [&]() -> sycl::event {
        (void)launch_sk<DpasS8Sc8w48m4skffNt2Name, 2, 5>(q, ad, bd, cpart, rows,
                                                       n, k);
        return launch_reduce(q, cpart, asd, bsd, cd, rows, n);
    };
...
int main(int argc, char **argv) {
    int nt = 2;
    int timed_m = 256, timed_n = 10240, timed_k = 5120;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8w48m4skff --nt 2 [--m 256] [--n 10240]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8w48m4skff: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2) {
        std::fprintf(stderr, "dpas_s8_sc8w48m4skff: --nt must be 2\n");
        return 2;
    }
    const int unroll = 5;
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s8xs8->f16_scaled RC=%d Mtiles=%d k128 NT=%d unroll=%d "
                "dpas=%d wg=%dx%d_NxM splitK=%d no_slm=1 "
