// K7 ESIMD GDN fused-recurrent delta T=256 skip-hi (gated b). Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// slmht leftover 260. Even-t b=0 skips vold+rank1. Rank pipe_host vs 260.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_skiphi: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaSkiphiName>(
...
#pragma unroll
                        for (int tile = 0; tile < kTiles; ++tile) {
                            const esimd::simd<float, kVl> snew = ah * sf[tile];
                            sf[tile] = snew;
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_skiphi: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
    ...
    for (int t = 0; t < tlen; ++t) {
        for (int h = 0; h < kNv; ++h) {
            ...
            hb[idx] = ((t % 2) == 0)
                          ? sycl::half(0.f)
                          : sycl::half(0.30f + float(h % 5) * 0.08f);
        }
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_skiphi [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_skiphi: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_skiphi dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d "
                "blk=%d skip_frac=0.5 even_t_b=0 warmup=%d iters=%d card=%d "
                "spin=%d heat=none\n",
