// K7 leftover: ESIMD s8 RC=4 NT=1, wg 4x2 along N, scale-to-f16.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: NT=1 wg 8x2 o-proj is 55 both cards vs W8A8 47 vs sc 62.
// 20 WGs. Steal smaller WG 4x2 (kWgN=8) -> 40 WGs at n=5120. Occupancy
// again. Rank pipe_host vs 55 and 47.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc_nt1w42: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc_nt1w42: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        return launch<DpasS8ScNt1w42Name, 1, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc_nt1w42 --nt 1 [--m 1] [--n 5120] [--k 6144]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc_nt1w42: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 1) {
        std::fprintf(stderr, "dpas_s8_sc_nt1w42: --nt must be 1\n");
        return 2;
    }
    const int unroll = 16;
