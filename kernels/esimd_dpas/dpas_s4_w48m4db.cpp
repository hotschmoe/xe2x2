// K2 ESIMD s4 4-acc + k64 A-db + wg 4x8 k128, f16, M=256, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s4 4-acc no A-db is 48.6 us at M=256 vs W8A8 75.
// s8 k32 A-db on this tile was a tax (135 vs 128). Steal k64 A
// ping-pong on s4: prologue A[k=0], issue A[k+64] before dpas.
// Same 4 M-tiles, wg 4x8, two s4 dpas per k128. Never E2M1.
// Rank pipe_host vs 48.6 and 75.
...
struct DpasS4W48m4dbNt2Name {};
struct DpasS4W48m4dbNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s4_w48m4db: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s4_w48m4db: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS4W48m4dbNt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                      rows, n, k);
        return launch<DpasS4W48m4dbNt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                  n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s4_w48m4db --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s4_w48m4db: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s4_w48m4db: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "dpas=%d wg=%dx%d_NxM A_double_buffer=1 no_slm=1 pack=2 "
