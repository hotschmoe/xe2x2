// K2 ESIMD s4 4-acc M-tile + wg 4x8 k128, f16, M=256, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: s8 4-acc wg 4x8 is 128 us at M=256 vs W8A8 75.
// s4 4x8 A-db was 33.6 us at M=64 (~2.24x s8 75). Steal native s4
// onto this 4-acc tile: packed 2/byte along K, two s4 dpas per
// k128 (s8 used four k32). Same 4 M-tiles, wg 4x8, no A-db, no SLM.
// Never E2M1 bitcast. s4 [-8,7]. Rank pipe_host vs 128 and 75.
...
struct DpasS4W48m4Nt2Name {};
struct DpasS4W48m4Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s4_w48m4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
                            for (int r = 0; r < kMTiles; ++r)
                                acc[r][t] = xmx::dpas<
                                    ...
                                    xmx::dpas_argument_type::s4>(acc[r][t], bt,
                                                                 a[r]);
                        }
                    }
                }
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "dpas_s4_w48m4: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS4W48m4Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasS4W48m4Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s4_w48m4 --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s4_w48m4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s4_w48m4: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s4xs4->f16_scaled RC=%d Mtiles=%d k128 NT=%d unroll=%d "
                "dpas=%d wg=%dx%d_NxM no_slm=1 pack=2 "
