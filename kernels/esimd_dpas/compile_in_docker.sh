#!/usr/bin/env bash
# CPU-only AOT for K2 ESIMD DPAS micros. No --device. No gpu-run.
...
LOGDIR="${ROOT}/results/k2"

mkdir -p "${ROOT}/kernels/esimd_dpas/bin" "${LOGDIR}"
...
set -u
cd /work/kernels/esimd_dpas
mkdir -p bin /work/results/k2
LOG=/work/results/k2/compile.log
: > "$LOG"
{
...
fail=0
for src in dpas_s8.cpp dpas_s4.cpp dpas_s2xs8.cpp dpas_s2.cpp; do
  stem="${src%.cpp}"
  dst="bin/${stem}"
  slog="/work/results/k2/${stem}.compile.log"
