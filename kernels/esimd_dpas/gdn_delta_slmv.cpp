// K7 ESIMD GDN fused-recurrent delta T=256 SLM-K + v-prefetch. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Serial S. Block-16 k/q in SLM. Prefetch 16 v into GRF.
// SLM-K 847 leftover. Rank pipe_host vs 847.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_delta_slmv: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const sycl::half *sd, const sycl::half *qd,
    ...
    return q.parallel_for<GdnDeltaSlmvName>(
...
#pragma unroll
                ...
                esimd::simd<sycl::half, kBlk> vv;
#pragma unroll
                for (int tt = 0; tt < kBlk; ++tt) {
                    const size_t vbase =
                        (size_t(t0 + tt) * size_t(kNv) + size_t(h)) *
                        size_t(kDv);
...
#pragma unroll
                    ...
                    const float vi = float(sycl::half(vv[tt]));
                    float oacc = 0.f;
#pragma unroll
                    for (int tile = 0; tile < kTiles; ++tile) {
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_delta_slmv: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_delta_slmv [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_delta_slmv: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_delta_slmv dtype=f16 T=%d nv=%d dv=%d dk=%d VL=%d blk=%d "
