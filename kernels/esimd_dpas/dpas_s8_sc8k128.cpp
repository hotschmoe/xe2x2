// K2 ESIMD s8 RC=8, k128 A-db, f16 scale, M=256, no SLM.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: M=64 load-path closed at sc8db 97-100 us. ngen M=256
// is wg 4x8 xaf k128, 384x dpas.8x8, no SLM, W8A8 75 us. Steal k128
// (4x k32) + A ping-pong on the 8-row tile. NT=2 U=8 -> 64 dpas.
// Rank pipe_host vs 75. Napkin: sc8db M=64 *4 ~392 us.
...
struct DpasS8Sc8k128Nt2Name {};
struct DpasS8Sc8k128Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc8k128: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
#pragma unroll
            for (int t = 0; t < NT; ++t)
                acc[t] = 0;
            esimd::simd<int8_t, kRc * kKc> a[kKPer];
#pragma unroll
            for (int i = 0; i < kKPer; ++i)
                a[i] = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
...
#pragma unroll
                for (int u = 0; u < kUnroll; ++u) {
                    ...
                    const int knext = more_u ? (k0 + kK128) : ((o + 1) * kInnerK);
                    const bool load_next = more_u || (o + 1 < outer);
                    esimd::simd<int8_t, kRc * kKc> an[kKPer];
                    if (load_next) {
#pragma unroll
                        for (int i = 0; i < kKPer; ++i)
                            an[i] = xesimd::lsc_load_2d<int8_t, kKc, kRc>(
                                ad, unsigned(dk - 1), unsigned(rows - 1),
                                unsigned(dk - 1), knext + i * kKc, row0);
                    }
#pragma unroll
                    for (int t = 0; t < NT; ++t) {
#pragma unroll
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc8k128: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Sc8k128Nt4Name, 4, 4>(q, ad, bd, asd, bsd, cd,
                                                      rows, n, k);
        return launch<DpasS8Sc8k128Nt2Name, 2, 8>(q, ad, bd, asd, bsd, cd, rows,
                                                  n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc8k128 --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc8k128: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc8k128: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "dtype=s8xs8->f16_scaled RC=%d k128 NT=%d unroll=%d dpas=%d "
                "wg=%dx%d_alongN A_double_buffer=1 no_slm=1 "
                ...
                kKPer * nt * unroll, kWgX, kWgY, double(kScale), warmup, iters,
                g_card, g_spin);
    int rc = 0;
    run_shape(q, nt, unroll, "check", kRc, nt * kExecN, unroll * kK128, 1, 1,
              &rc, 0);
