// K2 ESIMD DPAS s8 RC=8 with GRF256. Backend: sycl+l0. AOT intel_gpu_bmg_g31.
// Standalone icpx -fsycl (intel/llvm#21741).
//
// CONFIG prior: oneDNN W8A8 M=64 uses grf256. Same 8x16 tile as dpas_s8
// plus intelex::grf_size<256>. CSV: phase,m,n,k,us,TOPS,max_abs,ok
...
struct DpasS8Grf256Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_grf256: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const int8_t *ad, const int8_t *bd,
    ...
    return q.parallel_for<DpasS8Grf256Name>(
        sycl::nd_range<1>({threads}, {local}), props,
...
void run_shape(sycl::queue &q, const char *phase, int m, int n, int k,
    ...
    if (m % kRc != 0 || n % kExecN != 0 || k % kKc != 0) {
        std::fprintf(stderr, "dpas_s8_grf256: shape m=%d n=%d k=%d\n", m, n, k);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--m")
            ...
            std::fprintf(stderr,
                         "dpas_s8_grf256 [--m 64] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_grf256: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                "RC=%d K=%d N=%d grf=256 Bload=lsc_load_2d_Transformed "
                "warmup=%d iters=%d\n",
                backend, name.c_str(), driver.c_str(), kRc, kKc, kExecN, warmup,
                iters);
    std::printf("phase,m,n,k,us,TOPS,max_abs,ok\n");
    int rc = 0;
    run_shape(q, "check", kRc, kExecN, kKc, warmup, iters, &rc);
