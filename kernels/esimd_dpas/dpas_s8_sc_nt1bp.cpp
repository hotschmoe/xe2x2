// K7 leftover: ESIMD s8 RC=4 NT=1 + B software pipeline, scale-to-f16.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: NT=1 o-proj is 55 both vs W8A8 47. NT=2 B-pipeline is 67
// vs sc 62. Steal: NT=1 occupancy (20 WGs) plus prologue load next k64
// A/B into GRF before current dpas. Rank pipe_host vs 55 and 47.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc_nt1bp: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc_nt1bp: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        return launch<DpasS8ScNt1bpName, 1, 16>(q, ad, bd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc_nt1bp --nt 1 [--m 1] [--n 5120] [--k 6144]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc_nt1bp: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 1) {
        std::fprintf(stderr, "dpas_s8_sc_nt1bp: --nt must be 1\n");
        return 2;
    }
    const int unroll = 16;
