// K2 ESIMD s8 RC=4, 64 dpas.8x4, 8x2 along N, W8A8 scale epilogue to f16.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// CONFIG prior: held-2800 raw s32 is 36 us event / 36.4 us pipelined host.
// W8A8 K4 host 42-46 stores f16 with A_scale (M,1) * B_scale (1,N).
// ngen M=1 epilogue: f32 then mov :hf, store_block2d.ugm.d16.
// Steal: same wgn tile, acc * a_s * b_s, store half. Rank pipe_host vs 42-46
// at the same 2800 spin hold. Fill s8 in [-64,64] like the W8A8 bench.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_sc: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_sc --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_sc: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_sc: --nt must be 2 or 4\n");
        return 2;
    }
