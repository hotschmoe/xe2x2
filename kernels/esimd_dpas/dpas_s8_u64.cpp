// K2 ESIMD s8 RC=4, 64 static dpas.8x4. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx -fsycl (intel/llvm#21741).
//
// CONFIG prior: W8A8 M=1 ngen is 64x dpas.8x4 k64 wx4. k64 blocking
// left a K loop (4 or 16 dpas). Unroll k64 steps so 2*NT*UNROLL = 64.
// NT=4 UNROLL=8 (K inner 512); NT=2 UNROLL=16 (K inner 1024). No SLM.
...
struct DpasS8U64Nt2Name {};
struct DpasS8U64Nt4Name {};

sycl::device pick_device() {
    auto devs = sycl::device::get_devices(sycl::info::device_type::gpu);
    if (devs.empty()) {
        std::fprintf(stderr, "dpas_s8_u64: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m % kRc != 0 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "dpas_s8_u64: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8U64Nt4Name, 4, 8>(q, ad, bd, cd, m, n, k);
        return launch<DpasS8U64Nt2Name, 2, 16>(q, ad, bd, cd, m, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "dpas_s8_u64 --nt 2|4 [--m 4] [--n 5120] [--k 5120]\n");
            return 0;
        } else {
            std::fprintf(stderr, "dpas_s8_u64: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "dpas_s8_u64: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s dtype=s8xs8s32 "
                ...
                "innerK=%d Bload=lsc_load_2d_Transformed warmup=%d iters=%d\n",
