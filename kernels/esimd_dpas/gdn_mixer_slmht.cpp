// K7 ESIMD GDN mixer T=256: packed conv1d then slmht delta. Backend: sycl+l0.
// AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Packed q+k+v C=10240 conv over T, then slmht delta (L2 q/k, 16->48).
// CONFIG prior: packed mixer 1557 vs seq conv 38 + slmht 260 ~298.
...
struct GdnMixerSlmhtConvName {};
struct GdnMixerSlmhtDeltaName {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "gdn_mixer_slmht: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_conv(sycl::queue &q, const sycl::half *xd,
    ...
    return q.parallel_for<GdnMixerSlmhtConvName>(
...
sycl::event launch_delta(sycl::queue &q, const sycl::half *sd,
                         ...
                         int tlen) {
    return q.parallel_for<GdnMixerSlmhtDeltaName>(
...
void run_shape(sycl::queue &q, const char *phase, int tlen, int warmup,
    ...
    if (tlen < kBlk || (tlen % kBlk) != 0) {
        std::fprintf(stderr, "gdn_mixer_slmht: T=%d need multiple of %d\n", tlen,
                     kBlk);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--t")
            ...
            std::fprintf(stderr, "gdn_mixer_slmht [--t 256] [--spin 0]\n");
            return 0;
        } else {
            std::fprintf(stderr, "gdn_mixer_slmht: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "op=gdn_mixer_slmht conv+slmht dtype=f16 T=%d C=%d nv=%d "
