// K6 EXPLICIT NEGATIVE: bitcast E2M1 nibbles onto s4 DPAS. Expect numeric fail.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "bitcast_e2m1_s4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
int8_t bitcast_nibble_s4(uint8_t nib) {
    int8_t v = int8_t(nib & 15);
    if (v >= 8)
        v = int8_t(v - 16);
    return v;
}
...
void fill_e2m1_bitcast(int8_t *q, int8_t *bc, size_t n, unsigned seed) {
    for (size_t i = 0; i < n; ++i) {
        const uint8_t nib = uint8_t((i * 13u + seed) & 15u);
        q[i] = e2m1_q(nib);
        bc[i] = bitcast_nibble_s4(nib);
    }
}
...
void run_shape(sycl::queue &q, const char *phase, int m, int n, int k,
    ...
    if (m % kRc != 0 || n % kExecN != 0 || k % kKc != 0) {
        std::fprintf(stderr, "bitcast_e2m1_s4: shape m=%d n=%d k=%d not aligned\n", m, n,
                     k);
        *rc = 2;
        return;
    }
    ...
    std::vector<int8_t> hq(nb);
    fill_s4(ha.data(), na, 1);
    fill_e2m1_bitcast(hq.data(), hb.data(), nb, 9);
    pack_a(ha.data(), pa.data(), m, k);
    pack_b(hb.data(), pb.data(), k, n);
    host_s32(ha.data(), hq.data(), href.data(), m, n, k);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--check-m")
            ...
            std::fprintf(stderr,
                         "bitcast_e2m1_s4 [--check-m 8] [--check-n 16] [--check-k 64] "
                         "[--m 1024] [--n 512] [--k 1024] [--iters N] [--warmup W]\n");
            return 0;
        } else {
            std::fprintf(stderr, "bitcast_e2m1_s4: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
