// K6 serving-shaped NVFP4 two-launch VECTORIZED unpack on the s8 decode tile.
...
struct UnpackScvName {};
struct DpasS8Nt2Name {};
struct DpasS8Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "nibble_unpack_scv: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
inline esimd::simd<int8_t, kExecN>
decode_nibbles16(const esimd::simd<uint8_t, kExecN> &nib) {
    const esimd::simd<int16_t, kExecN> idx =
        esimd::convert<int16_t>(nib & uint8_t(7));
    esimd::simd<int16_t, kExecN> mag = idx;
    mag.merge(mag + int16_t(1), idx > int16_t(4));
    mag.merge(mag + int16_t(1), idx > int16_t(5));
    mag.merge(mag + int16_t(3), idx > int16_t(6));
    esimd::simd<int16_t, kExecN> q = mag;
    q.merge(-mag, (nib & uint8_t(8)) != uint8_t(0));
    return esimd::convert<int8_t>(q);
}
...
sycl::event launch_unpack(sycl::queue &q, const uint8_t *packed, int8_t *s8,
                          int k, int n) {
    const int rows = k / 2;
    const int n16 = n / kExecN;
    return q.parallel_for<UnpackScvName>(
        sycl::range<2>(size_t(rows), size_t(n16)),
        [=](sycl::id<2> id) SYCL_ESIMD_KERNEL {
            const int r = int(id[0]);
            const int c0 = int(id[1]) * kExecN;
            esimd::simd<uint8_t, kExecN> p;
            p.copy_from(packed + size_t(r) * size_t(n) + size_t(c0));
            const esimd::simd<int8_t, kExecN> qlo =
                decode_nibbles16(p & uint8_t(15));
            const esimd::simd<int8_t, kExecN> qhi =
                decode_nibbles16(p >> uint8_t(4));
            qlo.copy_to(s8 + size_t(2 * r) * size_t(n) + size_t(c0));
            qhi.copy_to(s8 + size_t(2 * r + 1) * size_t(n) + size_t(c0));
        });
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "nibble_unpack_scv: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto gemm = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasS8Nt4Name, 4, 8>(q, ad, bd, asd, bsd, cd, rows, n,
                                               k);
        return launch<DpasS8Nt2Name, 2, 16>(q, ad, bd, asd, bsd, cd, rows, n,
                                            k);
    };
    auto go = [&]() -> sycl::event {
        launch_unpack(q, pd, bd, k, n);
        return gemm();
    };
    ...
    const auto ctrl0 = std::chrono::steady_clock::now();
    for (int i = 0; i < iters; ++i)
        (void)gemm();
    q.wait_and_throw();
    const auto ctrl1 = std::chrono::steady_clock::now();
    const double s8ctrl_pipe =
        std::chrono::duration<double, std::micro>(ctrl1 - ctrl0).count() /
        double(iters);
    std::printf("s8ctrl_pipe_host_us,%.3f\n", s8ctrl_pipe);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "nibble_unpack_scv --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "nibble_unpack_scv: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "nibble_unpack_scv: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "arm=e2m1_nibble_unpack_scv two_launch simd_unpack never_bitcast_s4 packedB=K/2 "
