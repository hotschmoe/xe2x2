// K3/K6 E2M1 two-term s4 compose on the s4 4x8 A-db prefill tile.
...
// RC=8 wg 4x8 A-db scale-to-f16 tile (dpas_s4_db48). Rank pipe_host
// vs s4 33.6, 8x2-N compose 217.9, s8 75, W8A8 46.
...
struct DpasE2M1Db48Nt2Name {};
struct DpasE2M1Db48Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "compose_e2m1_db48: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "compose_e2m1_db48: shape m=%d n=%d k=%d nt=%d unroll=%d\n",
                     m, n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasE2M1Db48Nt4Name, 4, 8>(q, ad, bd_lo, bd_hi, asd,
                                                     bsd, cd, rows, n, k);
        return launch<DpasE2M1Db48Nt2Name, 2, 16>(q, ad, bd_lo, bd_hi, asd, bsd,
                                                  cd, rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "compose_e2m1_db48 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "compose_e2m1_db48: unknown arg %s\n",
                         a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "compose_e2m1_db48: --nt must be 2 or 4\n");
        return 2;
    }
