// K6 sparse bound: LO-ONLY s4 DPAS (drop hi). Expect numeric fail on +-8/12.
...
struct DpasE2M1ScNt2Name {};
struct DpasE2M1ScNt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "compose_e2m1_loonly: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "compose_e2m1_loonly: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasE2M1ScNt4Name, 4, 8>(q, ad, bd_lo, bd_hi, asd,
                                                   bsd, cd, rows, n, k);
        return launch<DpasE2M1ScNt2Name, 2, 16>(q, ad, bd_lo, bd_hi, asd, bsd,
                                                cd, rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr, "compose_e2m1_loonly --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "compose_e2m1_loonly: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "compose_e2m1_loonly: --nt must be 2 or 4\n");
        return 2;
    }
