// K8 serving-shaped NVFP4 spoof: packed E2M1 B, simd nibble LUT, s8 DPAS.
...
struct DpasLutU14Nt2Name {};
struct DpasLutU14Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "nibble_lut_sc_u14: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "nibble_lut_sc_u14: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasLutU14Nt4Name, 4, 8>(q, ad, pd, asd, bsd, cd, rows,
                                                n, k);
        return launch<DpasLutU14Nt2Name, 2, 14>(q, ad, pd, asd, bsd, cd, rows,
                                                n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "nibble_lut_sc_u14 --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "nibble_lut_sc_u14: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "nibble_lut_sc_u14: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "arm=e2m1_nibble_lut_sc_u14 never_bitcast_s4 packedB=K/2 "
