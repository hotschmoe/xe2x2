// K6 NVFP4 nibble LUT on the s8 4x8 A-db prefill tile.
...
//
// Tile: same as K2 dpas_s8_sc8db48. RC=8, wg 4x8 NxM, k64 A ping-pong,
// 64x dpas.8x8, scale-to-f16. B load is packed uint8 Transformed=false,
// simd LUT + VNNI4, then s8 DPAS. Rank pipe_host vs s8 75, LUT 8x2-N 656,
// W8A8 46, compose 4x8 68.7.
...
struct DpasLutDb48Nt2Name {};
struct DpasLutDb48Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "nibble_lut_db48: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "nibble_lut_db48: shape m=%d n=%d k=%d nt=%d unroll=%d\n",
                     m, n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasLutDb48Nt4Name, 4, 8>(q, ad, pd, asd, bsd, cd,
                                                    rows, n, k);
        return launch<DpasLutDb48Nt2Name, 2, 16>(q, ad, pd, asd, bsd, cd, rows,
                                                 n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "nibble_lut_db48 --nt 2|4 [--m 64] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "nibble_lut_db48: unknown arg %s\n",
                         a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "nibble_lut_db48: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "arm=e2m1_nibble_lut_db48 never_bitcast_s4 packedB=K/2 "
