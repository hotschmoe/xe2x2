// K6 serving-shaped NVFP4 two-launch unpack control on the s8 decode tile.
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "nibble_unpack_sc: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch_unpack(sycl::queue &q, const uint8_t *packed, int8_t *s8,
                          int k, int n) {
    const int rows = k / 2;
    const size_t np = size_t(rows) * size_t(n);
    return q.parallel_for<UnpackScName>(sycl::range<1>(np), [=](sycl::id<1> id) {
        const size_t i = id[0];
        const int r = int(i / size_t(n));
        const int c = int(i % size_t(n));
        const uint8_t p = packed[i];
        s8[size_t(2 * r) * size_t(n) + size_t(c)] =
            nibble_to_q(uint8_t(p & 15));
        s8[size_t(2 * r + 1) * size_t(n) + size_t(c)] =
            nibble_to_q(uint8_t(p >> 4));
    });
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0) {
        std::fprintf(stderr,
                     "nibble_unpack_sc: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m,
                     n, k, nt, unroll);
        *rc = 2;
        return;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "nibble_unpack_sc --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "nibble_unpack_sc: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "nibble_unpack_sc: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "arm=e2m1_nibble_unpack_sc two_launch never_bitcast_s4 packedB=K/2 "
