// K8 Lightning DOWN: two-term E2M1 compose on the s4 decode tile, kstep=64.
...
struct DpasE2M1K64Nt2Name {};
struct DpasE2M1K64Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "compose_e2m1_k64: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "compose_e2m1_k64: shape m=%d n=%d k=%d nt=%d unroll=%d\n", m, n,
                     k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasE2M1K64Nt4Name, 4>(q, ad, bd_lo, bd_hi, asd,
                                                 bsd, cd, rows, n, k);
        return launch<DpasE2M1K64Nt2Name, 2>(q, ad, bd_lo, bd_hi, asd, bsd, cd,
                                             rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr, "compose_e2m1_k64 --nt 2|4 [--m 1] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "compose_e2m1_k64: unknown arg %s\n", a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "compose_e2m1_k64: --nt must be 2 or 4\n");
        return 2;
    }
    const int unroll = 1;
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                "arm=compose_e2m1_k64 "
                "dtype=s4xE2M1_two_term->f16_scaled never_bitcast_s4 kstep=64 "
                "RC=%d NT=%d unroll=%d dpas_lo_hi_per_kstep=%d "
