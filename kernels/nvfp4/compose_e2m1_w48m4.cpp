// K3/K6 E2M1 two-term s4 compose on the s4 4-acc M=256 tile.
// Backend: sycl+l0. AOT intel_gpu_bmg_g31. Standalone icpx (intel/llvm#21741).
//
// Never bitcast E2M1 onto s4 DPAS. Split q=2*E2M1 to w_lo+8*w_hi,
// both in s4 [-8,7]. A is s4. Two s4xs4 DPAS per k64 on the K2
// RC=8 4-acc wg 4x8 k128 tile (dpas_s4_w48m4). Rank pipe_host vs
// s4 48.6, 4x8 compose 194.9, s8 128, W8A8 75.
...
struct DpasE2M1W48m4Nt2Name {};
struct DpasE2M1W48m4Nt4Name {};
...
sycl::device pick_device() {
    ...
    if (devs.empty()) {
        std::fprintf(stderr, "compose_e2m1_w48m4: no GPU\n");
        std::exit(2);
    }
    return devs[0];
}
...
sycl::event launch(sycl::queue &q, const uint8_t *ad, const uint8_t *bd_lo,
    ...
    return q.parallel_for<Name>(
        ...
        [=](sycl::nd_item<2> it) SYCL_ESIMD_KERNEL {
            ...
            esimd::simd<int32_t, kRc * kExecN> acc_lo[kMTiles][NT];
            esimd::simd<int32_t, kRc * kExecN> acc_hi[kMTiles][NT];
#pragma unroll
...
#pragma unroll
                            for (int r = 0; r < kMTiles; ++r) {
                                acc_lo[r][t] = xmx::dpas<
                                    ...
                                    xmx::dpas_argument_type::s4>(acc_lo[r][t],
                                                                 blo, a[r]);
                                acc_hi[r][t] = xmx::dpas<
                                    8, kRc, int32_t, int32_t, uint8_t, uint8_t,
                                    xmx::dpas_argument_type::s4,
                                    xmx::dpas_argument_type::s4>(acc_hi[r][t],
                                                                 bhi, a[r]);
                            }
                        }
                    }
...
#pragma unroll
                for (int t = 0; t < NT; ++t)
                    acc_lo[r][t] = acc_lo[r][t] + acc_hi[r][t] * 8;
...
#pragma unroll
                for (int t = 0; t < NT; ++t) {
                    ...
                    esimd::simd<float, kRc * kExecN> f(acc_lo[r][t]);
#pragma unroll
...
void run_shape(sycl::queue &q, int nt, int unroll, const char *phase, int m,
    ...
    if (m < 1 || n % tn != 0 || k % inner_k != 0 || k % kPack != 0) {
        std::fprintf(stderr,
                     "compose_e2m1_w48m4: shape m=%d n=%d k=%d nt=%d unroll=%d\n",
                     m, n, k, nt, unroll);
        *rc = 2;
        return;
    }
    ...
    auto go = [&]() -> sycl::event {
        if (nt == 4)
            return launch<DpasE2M1W48m4Nt4Name, 4, 4>(q, ad, bd_lo, bd_hi, asd,
                                                      bsd, cd, rows, n, k);
        return launch<DpasE2M1W48m4Nt2Name, 2, 8>(q, ad, bd_lo, bd_hi, asd, bsd,
                                                  cd, rows, n, k);
    };
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--nt")
            ...
            std::fprintf(stderr,
                         "compose_e2m1_w48m4 --nt 2|4 [--m 256] [--spin 4000]\n");
            return 0;
        } else {
            std::fprintf(stderr, "compose_e2m1_w48m4: unknown arg %s\n",
                         a.c_str());
            return 2;
        }
    }
    if (nt != 2 && nt != 4) {
        std::fprintf(stderr, "compose_e2m1_w48m4: --nt must be 2 or 4\n");
        return 2;
    }
    ...
    std::printf("# CONFIG backend=%s device=\"%s\" driver=%s "
                ...
                "RC=%d Mtiles=%d k128 NT=%d unroll=%d dpas_lo_hi=%d "
                "wg=%dx%d_NxM no_slm=1 pack=2 grf256_request=1 "
                "a_scale=b_scale=%.4f out=f16 warmup=%d iters=%d card=%d "
                "spin=%d heat=none\n",
                backend, name.c_str(), driver.c_str(), kRc, kMTiles, nt, unroll,
                2 * kMTiles * kKPer * nt * unroll, kWgX, kWgY, double(kScale),
